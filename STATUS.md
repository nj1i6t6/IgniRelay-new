...
## [2026-06-20] UI-H1 DONE — `我的` 加正式語言 / 字體大小設定（重用既有 root API）

- repo/commit: IgniRelay @ `b20dfe5`
- 執行者: Claude（Owner 授權 session）；依 `docs/UI_H_I18N_A11Y_PLAN.md`「UI-H1」+ `docs/UI_H_I18N_A11Y_AUDIT.md` §6 Q7/Q10 施工。**UI-H1 only，做完停，未順手做 H2/H4。**
- 範圍: 把既有但未對使用者開放的 locale / text-scale 設施接到 `我的` 分頁。**未碰 ARB / i18n 搬字串（H2）、未碰 legacy cleanup（H4）、未碰 wire/proto/GATT/crypto/DB/native；無新依賴、無新 singleton、無第二個 locale/設定 store。**
- 變更（4 檔 = 1 新 lib + 1 lib 接線 + 1 新 test + 1 test）:
  - 新 `lib/ui/shell/tabs/settings_section.dart`：**純展示 widget**（value+callback seam）。語言 中文/English（Stage A 不做「系統」跟隨，plan §H1）、字體大小 標準/大字/特大字/超大字（對應 `IgniTextScale`）。目前值由 `languageCode`/`textScale` 傳入、選擇以 `onLanguageSelected`/`onTextScaleSelected` 回呼出去；**刻意不 import main.dart、不持 state、不碰 SharedPreferences** ⇒ 互動可獨立 pump 測、不把啟動圖拉進單元測試。選中=品牌琥珀 chip、未選=中性，token-clean 0 Colors.*。
  - `lib/ui/shell/tabs/my_tab.dart`：於身分與角色卡下、權限狀態佔位上方掛 `SettingsSection`，接既有 root API——目前語言讀 `Localizations.localeOf(context).languageCode`（**無新增 locale getter**，audit Q7）、目前字級讀 `IgniRelayApp.textScaleOf(context)`、選擇交 `IgniRelayApp.setLocale`/`setTextScale`（持久化沿用 `app_language`/`app_text_scale`）。新增唯一 import `package:ignirelay_app/main.dart show IgniRelayApp`——經查 check_layers：main.dart 不在任何 forbidden prefix、靜態方法不觸 `Foo(` singleton 規則；main.dart top-level globals 為 lazy `final` ⇒ import 不會 eager 構造、測試無 plugin 風險。
- 測試（+9，684→693）:
  - 新 `test/ui/shell/settings_section_test.dart`（7，純 widget、零 provider）：渲染 設定/語言/字體大小 + 全部選項；語言選擇器反映現值（zh→中文 brand、en→English brand，用 `IgniChip.tone` seam 斷言選中）；字級選擇器反映現值；點語言/字級各自回呼正確 `Locale`/`IgniTextScale`；**選字級→選中狀態更新**（stateful host 鏡像正式接線：回呼驅動值→重繪新選中）；huge(1.45) 字級於 360 寬無 overflow。
  - `test/ui/shell/app_shell_test.dart`（+2，整合）：`我的` 分頁顯示 設定 區（語言/字體大小/中文/English/標準/超大字）；`我的` 於 `TextScaler.linear(1.45)` 無 overflow（MyTab 為 ListView，垂直可捲，守 row + 新 chip 的水平爆版）。
- DoD（對照 UI-H1）: ✅ 使用者可從正式 UI 改語言與字體大小；✅ 持久化走既有 key；✅ 無新 singleton；✅ 無新依賴；✅ 本 STATUS 記 gate 證據。Forbidden 全守：未引入第二 locale store、未全域偷 cap 字級、設定不在 DebugShell-only、未復活舊 profile/設定頁。
- gates（G17 逐字）:
  - `dart run tool/check_layers.dart --strict` → `ok — no boundary violations`，exit 0
  - `flutter analyze --no-fatal-infos --no-fatal-warnings` → `2 issues`（皆既有 baseline battery_optimization_guide.dart:27/249，0 新），exit 0
  - `flutter test --exclude-tags golden` → `All tests passed!` `+693 ~3`（UI-H0 基線 684 + 9 新），exit 0
  - `flutter test test/conformance/wire_conformance_corpus_test.dart` → `+21`（未動 wire），exit 0
  - GATE-KOTLIN-BUILD `:app:assembleDebugAndroidTest` → **BUILD SUCCESSFUL in 17s**（未碰 native，仍實跑），exit 0
  - grep gates：DESIGN §6 `Colors\.` 新檔 0 命中（settings_section/my_tab 僅註解文字提及）；`目前位置` 新檔 0；`import .../main.dart` 於 lib/ui 僅 my_tab 一處（sanctioned IgniRelayApp）。
- deviations: 無。**未宣稱 UI-H DONE、未宣稱 A11 passed。**
- next: **UI-H2**——將 §4 正式 UI 硬字串搬進 ARB（建議 H2a shell+設定 / H2b 場域+引導 / H2c 安全·事件·SOS，依 audit Q8 分刀、序列不並行）；其後 UI-H3 大字 QA → UI-H4 legacy cleanup → 回 A11-D2。

## [2026-06-20] UI-H2a DONE — AppShell / no-field entry / MyTab / SettingsSection i18n（zh/en）

- repo/commit: IgniRelay @ `4699067`
- 執行者: Claude（Owner 授權 session）；依 `docs/UI_H_I18N_A11Y_PLAN.md`「UI-H2」+ audit Q8 拆刀。**H2 最小首刀 H2a only，未做 H2b（場域/引導）/H2c（安全·事件·SOS）。**
- 範圍: 把正式殼最外圈硬字串搬進 ARB，使 AppShell 能 zh/en。**只動四個面：no-field entry、五分頁 label、MyTab、SettingsSection**；field/preview（H2b）、safety/events/SOS/cards（H2c）仍維持硬編。**未碰 wire/proto/GATT/crypto/DB/native；無新依賴；未動 main.dart `localeResolutionCallback`（預設語言規則不變：zh* 系統→zh、其餘→en，手動選擇仍持久化）。**
- 變更（10 檔 = 2 ARB + 3 generated + 3 lib + 2 test）:
  - `lib/l10n/app_zh.arb`（template）+ `app_en.arb`：新增 ~30 key——`shellTab*`(5 分頁 label)、`noField*`(標題/副標/三入口)、`my*`(我的：標題/副標/場域卡含 `myCurrentField{name}`·`myFieldJoinedCount{count}` placeholder/角色卡/權限佔位/開發者診斷)、`roleHost`/`roleMember`(主辦/成員)、`settings*`(設定/語言/字體大小/四段字級)。`flutter gen-l10n` 重生 `S`——**純新增（generated 三檔 204/108/106 行皆 0 刪除，無 churn）**。
  - `lib/ui/shell/app_shell.dart`：`const kAppShellTabLabels` → `List<String> appShellTabLabels(S l10n)`（保「單一真實來源＋固定順序」保證，順序由 ARB zh/en 值提供）；no-field 標題/副標/加入·建立·先看功能 改 `context.l10n`。
  - `lib/ui/shell/tabs/my_tab.dart`：header、場域卡、身分與角色卡、權限佔位、開發者診斷全 `context.l10n`；`_fieldCard`/`_roleCard` 加 `S l` 參數。
  - `lib/ui/shell/tabs/settings_section.dart`：設定/語言/字體大小/四段字級走 `context.l10n`；**「中文」「English」chip 維持字面 endonym**（兩語系皆以本名顯示，刻意不譯）。
- 測試（+4，693→697）:
  - `test/ui/shell/settings_section_test.dart`：harness 補 `S` delegates + locale 參數（預設 zh，舊 zh 斷言維持）；新增 en-locale 渲染測試（Settings/Language/Text size/Standard..Huge，且無「設定」「標準」）。
  - `test/ui/shell/app_shell_test.dart`：`_wrap`/`_pumpShell` 補 delegates + locale；`kAppShellTabLabels` 引用改為 `appShellTabLabels(lookupS(...))` 順序斷言 + 字面渲染；新增 3 個 en 測試（no-field 三入口英文/五分頁英文 label·無 安全·無 地圖·無 Map/我的設定區英文）。
- DoD（對照 UI-H2 §4 子集）: ✅ AppShell/no-field/五分頁/MyTab/SettingsSection 在 zh/en 皆正確；✅ app_zh.arb 與 app_en.arb 同步；✅ generated 已更新；✅ 本 STATUS 附 gate 證據。Forbidden 全守：未在地化 dead legacy 充當正式；未移除仍被編譯使用的 key；無未文件化的英文 fallback 硬編。
- gates（G17 逐字）:
  - `dart run tool/check_layers.dart --strict` → `ok — no boundary violations`，exit 0
  - `flutter analyze --no-fatal-infos --no-fatal-warnings` → `2 issues`（皆既有 baseline，0 新），exit 0
  - `flutter test --exclude-tags golden` → `All tests passed!` `+697 ~3`（UI-H1 693 + 4），exit 0
  - `flutter test test/conformance/wire_conformance_corpus_test.dart` → `+21`（未動 wire），exit 0
  - GATE-KOTLIN-BUILD `:app:assembleDebugAndroidTest` → **BUILD SUCCESSFUL**（未碰 native，仍實跑），exit 0
  - grep gates：H2a 三檔殘留 CJK 字面僅 `'中文'` endonym（其餘 0）；`目前位置` 0；`Colors\.` 新碼 0 實際使用（僅註解提及）。
- deviations: 無。**未宣稱 UI-H DONE、未宣稱 A11 passed。**
- next: **UI-H2b**——場域流程 + 引導預覽 i18n（`field_screen` / `field_scan_screen` / `field_qr_sheet` / 建立·加入·手動密鑰 dialog；`preview_screen` / `preview_fixtures`，含 fixtures 不得引入可解碼 secret/IGNI1/hex）；其後 H2c（安全·事件·SOS）→ UI-H3 大字 QA → UI-H4 legacy cleanup → 回 A11-D2。

## [2026-06-20] UI-H2b DONE — 場域流程 + 引導預覽 i18n（zh/en）

- repo/commit: IgniRelay @ `4539423`
- 執行者: Claude（Owner 授權 session）；依 `docs/UI_H_I18N_A11Y_PLAN.md`「UI-H2」+ audit Q8 拆刀，承 UI-H2a。**H2 第二刀 H2b only，未做 H2c（安全·事件·SOS）。**
- 範圍: 把場域加入流程與「先看功能」引導的硬字串搬進 ARB，使兩面能 zh/en。**只動五檔 UI**：`field_screen` / `field_scan_screen` / `field_qr_sheet`（加入/掃碼/代碼 dialog/建立 dialog/離開 dialog/QR sheet/錯誤訊息）、`preview_screen` / `preview_fixtures`（5 頁導覽 + 示範資料）。safety/events/SOS/cards（H2c）仍維持硬編。**未碰 wire/proto/GATT/crypto/DB/native；無新依賴；未動 main.dart `localeResolutionCallback`。**
- Owner 邊界①（preview fixtures 不得引入可解碼物）守住: fixtures 維持「結構性不可解碼」——無真 / 可解碼 join secret、QR payload、`IGNI1` 字串或 field_id 樣式 hex；`IGNI1` 在 preview 來源僅出現於兩行說明邊界的註解，無 payload 字面值。anon8 化名與示範座標皆顯示用、永不送入任何 publisher（preview 不持有 publisher/controller/location）。
- 變更（12 檔 = 2 ARB + 3 generated + 5 lib + 2 test）:
  - `lib/l10n/app_zh.arb`（template）+ `app_en.arb`：新增 ~98 key——`field*`(標題/副標/空狀態/三入口/已加入清單含 `fieldJoinedHeader{count}`/顯示 QR/離開/各建立·加入·離開 dialog/七類 `fieldErr*` 驗證訊息含 `fieldCreateFailed{error}` 等 placeholder)、`fieldScan*`(返回/標題/提示/拒絕/無相機)、`fieldQr*`(標題/副標/debug 警告/完成)、`preview*`(模式副標/badge/上下頁/示範 chip/5 頁文案/足跡相對時間/事件四類/`previewAlias{alias}` 化名)。`flutter gen-l10n` 重生 `S`——**純新增**。zh/en 鍵數對齊 1209=1209，雙向皆無缺鍵。
  - `lib/ui/screens/field/field_screen.dart`：`_activeCard`/`_actions`/`_roleChip`/`_fieldRow`/`_errorMessage` 加 `S l` 參數走 `context.l10n`；`_createField` 於 await 前先 `final l = context.l10n;`（修 use_build_context_synchronously）；複用 `roleHost`/`roleMember`/`myFieldUnnamed`；`field_id` 技術 label 維持字面。
  - `field_scan_screen.dart`：`_cameraError(p, S l, error)` 三類相機錯誤走 l10n。
  - `field_qr_sheet.dart`：build 讀 `context.l10n`，標題/副標/debug 警告/完成 走 l10n；`field_id` 維持字面。
  - `preview_fixtures.dart`：const 頂層資料改成吃 `S` 的 builder 函式（`previewFieldLabel(S)`/`previewFootprints(S)`/`previewRadarSubjects(S)`/`previewSos·Hazard·Broadcast·Checkpoint(S)`）；`kPreviewOrigin`/anon8/座標/tone 維持純資料。
  - `preview_screen.dart`：各頁維持 `const` 建構子，僅 build() 改讀 `context.l10n`；`_Header`/底部導覽鈕/`_JoinCtas`/`_EventRow._toneLabel`/`_FootprintRow` 與五頁全經 fixtures 函式。
- 測試（+2，697→699）:
  - `test/ui/screens/field_screen_test.dart`：`_wrap` 補 `S` delegates + `locale` 參數（預設 zh，舊斷言維持）；新增 en-locale 空狀態英文渲染測試。
  - `test/ui/screens/preview/preview_screen_test.dart`：`_previewApp` 補 delegates + `locale` 參數（**仍零 provider，import-guard 守的 fixture-only / zero-provider 性質不變**）；新增 en-locale badge+首頁英文測試。import-guard 拒名單不含 l10n import。
- DoD（對照 UI-H2 §4 子集）: ✅ 場域流程 + 引導預覽在 zh/en 皆正確；✅ app_zh.arb 與 app_en.arb 同步；✅ generated 已更新；✅ preview fixtures 維持不可解碼（Owner 邊界①）；✅ 本 STATUS 附 gate 證據。Forbidden 全守：未在地化 dead legacy 充當正式；未引入可解碼 secret/IGNI1/hex；無未文件化英文 fallback 硬編。
- gates（G17 逐字）:
  - `dart run tool/check_layers.dart --strict` → `ok — no boundary violations`，exit 0
  - `flutter analyze --no-fatal-infos --no-fatal-warnings` → `2 issues`（皆既有 baseline battery_optimization_guide.dart:27/249，0 新），exit 0
  - `flutter test --exclude-tags golden` → `All tests passed!` `+699 ~3`（UI-H2a 697 + 2），exit 0
  - `flutter test test/conformance/wire_conformance_corpus_test.dart` → `+21`（未動 wire），exit 0
  - GATE-KOTLIN-BUILD `:app:assembleDebugAndroidTest` → **BUILD SUCCESSFUL in 5s**（未碰 native，仍實跑），exit 0
  - grep gates：五檔殘留 CJK 字面值 0（全部 CJK 命中皆在 `//`/`///` 註解）；`目前位置` 0 字串（唯一命中為 preview_screen.dart:14 規則說明註解）；`Colors\.` 新碼 0；preview 來源 `IGNI1` 0 payload（僅 2 行邊界註解）。
- deviations: 無。**未宣稱 UI-H DONE、未宣稱 A11 passed。**
- next: **UI-H2c**——安全·事件·SOS 分頁與卡片 i18n（safety/events tabs、SOS 流程、hazard/checkpoint/broadcast cards）；其後 UI-H3 大字 QA → UI-H4 legacy cleanup → 回 A11-D2。

## [2026-06-20] UI-H2c DONE — 安全 / 事件 / SOS / 卡片 / 雷達 i18n（zh/en）

- repo/commit: IgniRelay @ `cbba564`
- 執行者: Claude（Owner 授權 session）；依 `docs/UI_H_I18N_A11Y_PLAN.md`「UI-H2」+ audit Q8 拆刀，承 UI-H2a/H2b。**H2 最後一刀（最後一批正式 UI i18n）。**
- 範圍: 把正式產品殼剩餘硬字串搬進 ARB，使能 zh/en。**只動 Owner 指定的十檔 UI**：`safety_tab` / `communication_state`（顯示文案搬到 seam，見下）/ `events_tab` / `assist_tab` / `hazard_card` / `checkpoint_card` / `admin_broadcast_banner` / `sos_screen` / `last_seen_screen` / `relative_radar`。**未碰 wire/proto/GATT/crypto/DB/native；無新依賴；未動 main.dart `localeResolutionCallback`。**
- communication_state 純 Dart 邊界（Owner 硬要求）: `communication_state.dart` **不 import l10n、維持純 Dart**；移除 `bestPathLabel`/`cloudLabel` 兩個 zh 顯示 getter，改由 `SafetyTab` render seam 以 `bestPath`(enum)/`cloudConfigured`(bool) → l10n 產生（`_bestPathLabel`/`_cloudLabel`）。**Stage A「雲端只設定、永不宣稱可達/已連線」誠實不變式**從純 getter 改在 l10n 層守：`communication_state_test` 對 zh+en 兩語系斷言 `cloudOffline`/`cloudConfigured` 皆不含 已連線/可達/connected/reachable，且 configured 仍標「尚未啟用 / not active」。
- SOS 語意保留（Owner 硬要求）: `sos_screen` 純搬 i18n、**不改任何措辭語意**——受困/受傷 severity、倒數、我安全了、各 outcome 行皆 1:1 對映，未動 SosController 行為。
- 變更（22 檔 = 2 ARB + 3 generated + 10 lib + 7 test）:
  - `app_zh.arb`（template）+ `app_en.arb`：新增 ~142 key——`safety*`/`commsPath*`/`cloud*`/`gps*`/`beacon*`/`motion*`、`events*`、`assist*`、`hazardCard*`（避開既有 dead `hazard*` map-era key）、`checkpointCard*`、`admin*`、`sos*`、`lastSeen*`/`confidence*`、`radarCaption`，外加共用 `commonSend`/`noCoordinate(Paren)`/`timeAgo*`/`timeJustNow`（重用既有 `commonCancel`）。`flutter gen-l10n` 重生 `S`——**純新增**；**zh 值與原字面值逐字一致**（含全形冒號「：」等），故既有 zh 斷言（如 app_shell_test 的「動作偵測：尚未啟用」「GPS 定位：尚無定位」）續綠。zh/en 鍵數對齊 1351=1351。
  - 十個 UI 檔：硬字串改 `context.l10n`；`safety_tab` 加 comms/cloud enum→l10n seam helper；`relative_radar` 唯一 caption；技術 label（`field_id` 樣式 `checkpoint_id`、`sev N`、`SOS`、`N`）維持字面。
  - hazard/checkpoint/admin 三卡的既有 raw-Material 樣式（`Colors.grey/amber`）**原封不動**——這些 debug-origin 卡的 DESIGN token 化屬後續 polish、非本刀；**淨零新增 Colors.*（逐卡 HEAD==now：2/3/4）**。
- 測試（+8，699→707）:
  - 六個 widget harness（sos/last_seen/relative_radar/hazard_card/checkpoint_card/admin_broadcast_banner）補 `S` delegates + `locale` 參數（預設 zh，舊斷言維持）+ 各加一個 en-locale smoke。
  - `communication_state_test`：移除已搬走的 label 字串斷言、保留 enum 推導 + `cloudConfigured` passthrough；新增 zh+en 雲端誠實 l10n 守衛。
- DoD（對照 UI-H2 §4 子集）: ✅ 安全/事件/SOS/卡片/雷達在 zh/en 皆正確；✅ app_zh.arb 與 app_en.arb 同步、generated 已更新；✅ communication_state 維持純 Dart、誠實不變式有測試守；✅ SOS 語意未變；✅ 本 STATUS 附 gate 證據。Forbidden 全守：未碰 H3 大字 layout、未碰 H4 legacy cleanup、未碰 wire/proto/GATT/DB/native。
- gates（G17 逐字）:
  - `dart run tool/check_layers.dart --strict` → `ok — no boundary violations`，exit 0
  - `flutter analyze --no-fatal-infos --no-fatal-warnings` → `2 issues`（皆既有 baseline battery_optimization_guide.dart:27/249，0 新），exit 0
  - `flutter test --exclude-tags golden` → `All tests passed!` `+707 ~3`（UI-H2b 699 + 8），exit 0
  - `flutter test test/conformance/wire_conformance_corpus_test.dart` → `+21`（未動 wire），exit 0
  - GATE-KOTLIN-BUILD `:app:assembleDebugAndroidTest` → **BUILD SUCCESSFUL in 4s**（未碰 native，仍實跑），exit 0
  - grep gates：十檔殘留 CJK 字面值 0（全部 CJK 命中皆在 `//`/`///` 註解）；`目前位置` 0 字串（唯二命中為 last_seen/relative_radar 規則說明註解）；六個 token-clean 螢幕/分頁 `Colors\.` 新碼 0（僅註解提及）；三卡 `Colors.` 淨零新增。
- deviations: 無。**未宣稱 UI-H DONE、未宣稱 A11 passed。**
- next: **UI-H + 正式 UI i18n 三刀（H2a/H2b/H2c）全數收尾**；下一棒 **UI-H3 大字 QA**（標準/大字/特大字/超大字下逐畫面查 overflow，必要時調 layout）→ UI-H4 legacy cleanup（移除確認 dead 的舊 `map*`/`hazardDialog*` 等 ARB/碼）→ 回 A11-D2 USER-GATE。

## [2026-06-20] UI-H3 DONE — 大字級 / 文字縮放 QA + 爆版修補（5 關鍵畫面）

- repo/commit: IgniRelay @ `70f02ab`
- 執行者: Claude（Owner 授權 session）；依 `docs/UI_H_I18N_A11Y_PLAN.md`「UI-H3」+ Owner 口頭範圍（專心測 1.45 app 字級與 effective 2.0 composite，修爆版/按鈕文字，SOS/Field/Preview/Safety/LastSeen 五畫面）。**H3 only，未進 H4。**
- 範圍: 五個關鍵畫面在 **app 字級 1.45** 與 **effective 2.0 composite（system×app 上限）**、**360 窄寬** surface 下做 overflow QA 並修爆版。**純 layout 修補**——未改文案、未動 ARB、未動 token/顏色、未碰 wire/proto/GATT/crypto/DB/native；不做 H4 legacy cleanup。
- 方法: 每個畫面在 `MediaQuery` textScaler 1.45 / 2.0 下、`tester.view.physicalSize = 360×820` 上 pump，斷言主要狀態無 `RenderFlex` overflow（`tester.takeException() == null`）。逐 overflow 用 probe dump 出 `error-causing widget` 的 file:line 精準定位再修。
- 找到並修掉的三處實際爆版（皆 layout-only）:
  - `safety_tab` 通訊卡：狀態標題與開關鈕同一 Row、2.0 時把鈕擠出右緣（85px）→ 標題改 `Expanded`（maxLines 2 + ellipsis），鈕保留位置。
  - `last_seen_screen` 估計卡：化名+SOS chip+可信度 chip 同列、以及 年齡/誤差 meta 列 overflow（65/46px）→ 化名群組改 `Expanded` 內 `MonoText` ellipsis，meta 改 `Wrap`。
  - `field_screen` 作用中卡：`field_id` 等寬值列無 flex、1.45 時 overflow ~9px → `MonoText` 改 `Expanded` + ellipsis。
  - 設計觀察：`IgniButton` 已用 `Flexible`+ellipsis 綁定 label，故「按鈕文字」本身不會水平爆版；爆版都出在**未包 flex 的相鄰 Row 元素**。
- SOS / Preview：兩畫面在 1.45 / 2.0 皆已穩健（含 SOS 倒數卡、Preview 五頁含雷達頁與事件/足跡列），**只加壓力測試、無需改 layout**。
- 測試（+5，707→712）:
  - `sos_screen_test`（trigger + countdown @ 1.45/2.0）、`app_shell_test`（安全分頁 @ 1.45/2.0）、`last_seen_screen_test`（估計卡 @ 1.45/2.0）、`field_screen_test`（作用中+已加入列、長場域名 @ 1.45/2.0）、`preview_screen_test`（走完五頁 @ 1.45/2.0，維持 zero-provider；以 per-scale `ValueKey` 確保 PageController state 不跨 scale 殘留）。
- DoD（對照 UI-H3）: ✅ 五畫面在 1.45 與 effective 2.0 無 overflow；✅ 修補僅動 layout、文案/ARB/顏色不變；✅ 壓力測試入庫防迴歸；✅ 本 STATUS 附 gate 證據。Forbidden 全守：未改文案語意、未進 H4、未碰 wire/proto/GATT/DB/native。
- gates（G17 逐字）:
  - `dart run tool/check_layers.dart --strict` → `ok — no boundary violations`，exit 0
  - `flutter analyze --no-fatal-infos --no-fatal-warnings` → `2 issues`（皆既有 baseline battery_optimization_guide.dart:27/249，0 新），exit 0
  - `flutter test --exclude-tags golden` → `All tests passed!` `+712 ~3`（UI-H2c 707 + 5），exit 0
  - `flutter test test/conformance/wire_conformance_corpus_test.dart` → `+21`（未動 wire），exit 0
  - GATE-KOTLIN-BUILD `:app:assembleDebugAndroidTest` → **BUILD SUCCESSFUL in 4s**（未碰 native，仍實跑），exit 0
  - grep gates：三個改動螢幕新增 `Colors\.` 0；新增非註解 CJK 字面值 0（CJK 僅出現在 `// UI-H3` 註解）；未動任何 ARB。
- deviations: 無。**未宣稱 UI-H DONE、未宣稱 A11 passed。** 註：events/assist 分頁與 hazard/checkpoint/admin 三卡未納入本刀壓測（非 Owner 點名的關鍵畫面；其卡片 raw-Material 樣式整理本就排後續 design-token polish）。
- next: **UI-H4 legacy cleanup**（移除確認 dead 的舊 `map*` / `hazardDialog*` 等 ARB key 與對應死碼；需先以 grep/usage 確認零引用再刪）→ 回 A11-D2 USER-GATE（Owner 雙機回填 13 步）。

## [2026-06-20] UI-H3-polish DONE — 補齊 §UI-H3 必測畫面（no-field / AppShell bottom nav / MyTab）@ 2.0
- repo/commit: IgniRelay @ `00c6d1e`
- 執行者: Claude（Owner 本 session 接任主理）。依 Owner 指示與 `docs/UI_H_I18N_A11Y_PLAN.md` §UI-H3「Required Screens / Tests / DoD」：首刀 UI-H3（`70f02ab`）只壓測 SafetyTab/Field/Preview/LastSeen/SOS，文件列的 **no-field entry、AppShell bottom navigation、MyTab settings**、以及「no-field + global SOS reachability under composite stress」尚未覆蓋。本刀補齊。**先不進 H4。**
- 範圍: 補測為主；發現 overflow 才**局部** Flexible/Expanded/Wrap 修補（Owner 紅線）。**未動 ARB / 文案 / wire / proto / GATT / DB / native；未全域 cap text scale；未隱藏 SOS 或三入口。**
- 測試（`app_shell_test.dart`，+3）:
  - no-field entry 在 1.45 / 2.0（360 寬）survive，加入/建立/先看功能 三入口仍在、`takeException()==null`；
  - joined AppShell bottom nav @ 2.0：五分頁 label 皆在、每切一個 tab global SOS（`kGlobalSosButtonKey`）仍可達且該 tab body 不爆版（IndexedStack 同時建構五分頁 → 單次 pump 壓全部，並逐分頁切換驗證）；
  - MyTab settings @ 2.0：`SettingsSection` + 設定/語言/字體大小/中文/English/超大字 皆可達（`ensureVisible` 最底 chip）、`takeException()==null`。`_pumpShell` 加可選 `textScale` 參數，以真殼（NoFieldEntry / 五分頁 / 全域 SOS）壓測而非孤立分頁。
- 修補（三處實際 overflow，皆 layout-only）:
  - `my_tab` field 卡 shortId + 已加入計數 Row → `Wrap`（等寬 id 不可斷行；93px）；
  - `checkpoint_card` / `hazard_card` header Row → 標題 `Expanded`(ellipsis) + 動作鈕改 `Flexible` 且 label ellipsis（**Material 按鈕 padding 隨字級放大**，2.0 時鈕本身就超寬，567/123px；只綁標題不足，必須讓鈕可縮）。三卡仍為既有 raw-Material 樣式（DESIGN token 化仍排後續 polish），本刀只解 overflow、未加新 `Colors.`（HEAD==now 2/3/1）。
- DoD（對齊 §UI-H3）: ✅ no-field 三入口在 huge/composite 仍全在；✅ global SOS 在 2.0 各 tab 仍可達；✅ AppShell bottom nav / MyTab settings 不爆版；✅ 字級選項皆可達；✅ 未動 `IgniTextScale.huge`；✅ 未移除產品關鍵文案（標題在 2.0 stress 上界 ellipsis、動作鈕 icon+tap 仍在，符合「composite 保關鍵動作可見＋不崩」）。
- gates（G17 全綠）:
  - `dart run tool/check_layers.dart --strict` → `ok — no boundary violations`，exit 0
  - `flutter analyze --no-fatal-infos --no-fatal-warnings` → `2 issues`（皆 baseline battery_optimization_guide.dart:27/249，0 新），exit 0
  - `flutter test --exclude-tags golden` → `All tests passed! +715 ~3`（UI-H3 712 + 3），exit 0
  - `flutter test test/conformance/wire_conformance_corpus_test.dart` → `+21`（未動 wire），exit 0
  - GATE-KOTLIN-BUILD `:app:assembleDebugAndroidTest` → **BUILD SUCCESSFUL in 19s**，exit 0
  - grep gates：三改動檔新增 `Colors\.` 0（net-zero 2/3/1）、新增非註解 CJK 0、`目前位置` 僅 §3.6 規則註解（未動位置文案），未動 ARB
- deviations: 無。**未宣稱 UI-H DONE、未宣稱 A11 passed。**
- next: **UI-H4 legacy cleanup**（待 Owner 明確發落；移除確認 dead 的舊 `map*`/`hazardDialog*` 等 ARB key 與死碼，先 grep/usage 證零引用再刪）→ A11-D2 USER-GATE（Owner 雙機回填 13 步，AI 可代跑 ADB 但不得代填/宣稱通過）。

## [2026-06-21] A11-fix-prep DONE — 補齊 A11 實測暴露的兩個 read-model 缺口 + 釐清包名（不碰 wire/proto/GATT/crypto；不宣稱 A11 通過）

- repo/commit: IgniRelay @ `732a4a1`
- 執行者: Claude（Owner 授權 session）；Owner 帶另一個 AI 的測試指令，並要求**先核對程式碼確認問題是否真的存在**再修（「不要辜負信任」）。經靜態閱讀 + 連線雙機 `pm list packages` 實測確認：兩個 UI/read-model 缺口屬實，第三項「包名污染」實為**舊版殘留非 gradle 問題**。
- 範圍: 只動 UI / read-model facade，**未碰 wire/proto/GATT/crypto/DB schema/native/pubspec**；wire conformance corpus 維持 +21 不變。
- 缺口 1（屬實）— LastSeenScreen 未處理 SOS 解除（SAFE）:
  - 原本只訂閱 sosAlerts/presence/checkpoint，**未訂閱** `EventStream.sosResolutions`，故求救者送 SAFE 後其 SOS 點/列仍永久顯示。`SosResolved.authorKeyHex` = `sender_pub_key` hex，正好是 `_sos` 既有 key。
  - 修：加可選 `sosResolvedSource` seam（預設 `sosResolutions`）；`_onSosResolved` 以 authorKeyHex 移除 `_sos` + `_sosLabel`（空 hex/未知 author no-op；他人 SAFE 不誤清）；dispose 取消訂閱。
- 缺口 2（屬實）— HazardCard 只靠 live `hazardEvents`:
  - `_hazards` 僅由 broadcast stream 餵，broadcast **不重播**，故「事件」分頁 mount 前就收到的 HAZARD 不顯示。
  - 修：新 `EventStream.recentHazards({limit})`（查 `Event_Logs` 的 `hazardMarker` 經 EventDecoder 還原；UI 仍不碰 app/proto·mesh·db）；HazardCard 加 `hazardBackfill` seam（預設 recentHazards），mount 時讀一次補齊並以 eventId 去重，live stream 續追加。
- 缺口 3（非 gradle 問題，已釐清）— connectedDebugAndroidTest 包名:
  - applicationId = `network.ignirelay.field`、無 `testApplicationId`/`applicationIdSuffix`；gate 安裝 app-under-test `network.ignirelay.field` + 暫存 instrumentation `network.ignirelay.field.test`，**不會**產生 bare `network.ignirelay`。
  - 雙機實測：手機 `94067a07` 無殘留；手機 `LNZ5TKY5NRNVQ4K7` 殘留 bare `network.ignirelay` **v0.2.0（2026-05-27 安裝）**＝包名改名前的舊版安裝、非本 gate 污染。已 `adb uninstall network.ignirelay` 清除，現該機僅 `network.ignirelay.field`。
  - runbook 修：§2 加舊 bare 包卸除；Step 12 加 gate 後 `pm list packages | findstr ignirelay` 查核（應僅 `.field`〔+ 可選 `.field.test`〕）。
- 變更檔（7 = 3 lib + 3 test + 1 doc，+322/−26）:
  - lib: `event_stream.dart`（+recentHazards）、`last_seen_screen.dart`（+sosResolvedSource/_onSosResolved/dispose）、`hazard_card.dart`（+hazardBackfill/_runBackfill/_mergeHazard + doc）。
  - test（+4）: last_seen（SAFE 清同 author SOS、他人 SAFE 不清）、hazard_card（mount 前已存在 backfill + live 追加）、event_stream（recentHazards 讀回已存 hazardMarker，真 in-memory DB+decoder）。
  - doc: `ACCEPTANCE_A11_TWO_PHONE_SCRIPT.md` Step 8/Step 9/§2/Step 12/彙總表/§8。
- gates（G17 逐字）:
  - `dart run tool/check_layers.dart --strict` → `ok — no boundary violations`，exit 0
  - `flutter analyze --no-fatal-infos --no-fatal-warnings` → `2 issues`（皆既有 baseline battery_optimization_guide.dart:27/249，0 新），exit 0
  - `flutter test --exclude-tags golden` → `All tests passed!` `+719 ~3`，exit 0
  - `flutter test test/conformance/wire_conformance_corpus_test.dart` → `+21`（未動 wire），exit 0
  - GATE-KOTLIN-BUILD `:app:assembleDebugAndroidTest` → **BUILD SUCCESSFUL**（未碰 native，仍實跑），exit 0
- 驗證方法誠實聲明: 兩個 UI 缺口以**靜態閱讀 + 新增 widget/unit 測試**證實並覆蓋（決定性、可重跑）；包名以**連線雙機 adb 實測**確認。**未進行雙機互傳功能性實測**（屬 A11-D2 USER-GATE，需 Owner 回填）。
- deviations: 無。
- next: **A11-D2（USER-GATE，今晚）**——Owner 兩台 Android 實機照 runbook 回填；本刀只是修好缺口讓實測更有意義，**不代表 A11 通過**。

## [2026-06-21] A11-debug-1-fix DONE — SOS/SAFE 送出帶真實 last-known 座標（送端取位鏈；不碰 wire/proto）

- repo/commit: IgniRelay @ `4e5a278`
- 執行者: Claude（Owner 授權 session）；A11 白天 debug 階段（無手機，純程式修補讓晚上可實測）。經 A11-debug-1 read-only 界定確認**非 wire/proto 問題**——`StatusUpdateData.location`（proto3 field 3，凍結）encode/decode/投影皆支援；座標在送端 `LocationEvidenceBuilder.build()` 變 null。
- 根因（送端 plumbing，兩處）:
  - `SosController._send()` 雖 `await _ensureFresh()`，但 hook 回傳 `void`，**丟棄** `ensureFreshForManualEvent` 已解析的 `LatLng?`（fresh-or-last-known），再各自 `build()` 重讀 `currentLocation` → 兩步走不同抽象。
  - bounded refresh 僅 `getCurrentPosition`（SOS 1500ms timeout，實機常逾時）；逾時且 app `currentLocation` 為 null 時，**從不查 OS `getLastKnownPosition`**（那「1 分鐘前的 fix」）→ 送 null。PRESENCE 倖免＝其 beacon refresh 用 8s timeout。
- 範圍: 只動送端取位鏈，**未碰** HAZARD / secure_storage / LastSeen hydrate / runbook / wire / proto / canonical / conformance corpus；無新 singleton；UI 不直接碰 LocationService。wire corpus 維持 +21。
- 修:
  - `SosController.ensureFreshLocation` 改回傳 `Future<LatLng?>`（不再丟棄 fix）；新增 `lastKnownLocation` seam；新增 `_resolveSendLocation()`——取位優先序 **fresh-or-last-known fix → builder 來源(currentLocation) → OS last-known**，全無回 null（仍送、**永不假座標**）。last-resort seam 包 try/catch → throw 視為 null，不阻止送出（zero-delay 紅線）。`_send()`/`markSafe()` 改用它。
  - `LocationService.lastKnownPosition()`：純加法 OS last-known passthrough（包既有 getLastKnownFn seam；**不** adopt → fix-age 誠實），**僅** SOS provider 接線使用，HAZARD/CHECKPOINT 行為零變動。
  - `main.dart`：只動 SOS provider（hook 回傳 fix + 接 lastKnownLocation）。
- 變更檔（4 = 3 lib + 1 test，+242/−20）: `sos_controller.dart` / `location_service.dart` / `main.dart` / `sos_controller_test.dart`。
- test（+7）: fresh fix 帶且 lat/lng 正確、無 fix 送 null（不假）、markSafe 有 fix 也帶、refresh 失敗回退 builder last-known、refresh+builder 空但 OS last-known 有→用之（實機那案）、三者全空送 null 不 throw、**last-known seam throw 不阻止 SOS**。
- gates（G17 逐字，全綠）:
  - `dart run tool/check_layers.dart --strict` → `ok — no boundary violations`，exit 0
  - `flutter analyze --no-fatal-infos --no-fatal-warnings` → `2 issues`（皆 baseline battery_optimization_guide.dart:27/249，0 新），exit 0
  - `flutter test --exclude-tags golden` → `All tests passed! +726 ~3`，exit 0
  - `flutter test test/conformance/wire_conformance_corpus_test.dart` → `+21`（未動 wire），exit 0
  - GATE-KOTLIN-BUILD `:app:assembleDebugAndroidTest` → **BUILD SUCCESSFUL**（未碰 native，仍實跑），exit 0
- 驗證方法誠實聲明: 取位鏈邏輯以**新增 unit 測試（注入 seam）**證實並覆蓋（決定性、可重跑）；**OS-last-known 那條的實機效果**（「1 分鐘前 fix 卻送 null」）需晚上雙機重現確認。
- deviations: 無。**未宣稱 A11 通過。**
- next: **A11-debug-2**——查 HAZARD 重啟後事件頁重複列（疑回填/dispatch dedup 或 DB 雙列；read-only 界定先行）。

## [2026-06-21] A11-debug-2-fix DONE — 停掉 HAZARD v1 dual-write（v2-only；不碰 wire/proto）

- repo/commit: IgniRelay @ `c5facde`
- 執行者: Claude（Owner 授權 session）；A11 白天 debug。經 A11-debug-2 read-only 界定確認「重啟後 HAZARD 兩列」是 **DB 層重複**＝同一危害被 v1 + v2 兩條線各落一筆 `Event_Logs` `hazardMarker`、`event_id` 不同（v1 id + `"v2-"+hex` 投影 id），無 id 共用鍵 → `recentHazards` 兩列都回。排除 UI 同 id 重加（`_mergeHazard` 以 eventId 去重）、排除重啟重投影（`ingestVerifiedEvent` 有 DB 層 event_id 去重）。projector 檔頭「DOUBLE DISPLAY DURING DUAL-WRITE」早已預言此狀況。
- 決策: Owner 選 **Option B（源頭砍重複）**，不選顯示層 dedup（A 會留髒 DB，事件頁/後台/同步/hydrate 任一再讀又遇到）；不加 sender self-projection（要做漂亮需 v2 facade 回傳 envelope id / 正式 local projection API，另開設計，不在本刀）。
- 修: `EventPublisher.publishHazard` 改 **v2-only**——移除 `_em.publishHazard`（v1）呼叫，只走 v2 `publishHazardMarker`（rename `_dualWriteHazardMarker`→`_publishHazardMarkerV2`）。**必須整段移除而非只停 wire enqueue**：v1 path 的本機 `Event_Logs` 列（非 `"v2-"` id）會被 v1 bloom/IBLT 同步廣告，留著本機列重複仍會經同步再傳到 peer。
- 範圍: **不碰** wire/proto/canonical/corpus（只停「發送 legacy 副本」）、SOS/SAFE（`publishEvent` 仍 dual-write status，未動）、secure_storage、LastSeen hydrate、runbook。三處過時註解一併更新（event_publisher 檔頭、v2 facade dual-write entry list、v2_inbound_projector migration note）。`HazardManager`/`EventManager.publishHazard` 保留（仍被 event_manager_test 覆蓋，只是 EventPublisher 不再呼叫）。
- 變更檔（4，+68/−37）: `event_publisher.dart` / `event_publisher_v2_facade.dart`（註解）/ `v2_inbound_projector.dart`（註解）/ `event_publisher_dual_write_test.dart`（測試改 v2-only）。
- test: dual-write 測試改成 v2-only expectation 並**強化**（斷言 publishHazard 後 `Hazards_State` 與 `Event_Logs` hazard 列**皆空**＝v1 已移除），v2 `publishHazardMarker` 參數斷言全保留不弱化；`publishEvent` dual-write 測試不變（確認 SOS/status 未受影響）。
- **刻意行為變更（記錄）**:
  - **HAZARD v1-only peer interop intentionally dropped as part of the v2 migration.**
  - **Sender no longer self-lists its own HAZARD in the 事件 tab**（v1 path 是唯一的本機 read-model 寫入；v2 送出不 self-project，與 SOS 一致）。
  - HAZARD 不再經 EventManager rate-limit；`radiusMeters` 不再持久化（v2 payload 無 radius，mapless UI 也不顯示）。
  - **既有裝置 DB 的舊 dual-write 雙列不會被本刀遷移／清除**；**今晚 A11 驗收必須 clean install / clear app data 再測**，否則仍會看到歷史殘列。
- gates（G17 逐字，全綠）:
  - `dart run tool/check_layers.dart --strict` → `ok — no boundary violations`，exit 0
  - `flutter analyze --no-fatal-infos --no-fatal-warnings` → `2 issues`（皆 baseline battery_optimization_guide.dart:27/249，0 新），exit 0
  - `flutter test --exclude-tags golden` → `All tests passed! +726 ~3`，exit 0
  ...
  - GATE-KOTLIN-BUILD `:app:assembleDebugAndroidTest` → **BUILD SUCCESSFUL**（未碰 native，仍實跑），exit 0
- deviations: 無。**未宣稱 A11 通過。**
- next: 回 **A11-debug** 籃子剩項——secure_storage `allowBackup="false"`、LastSeen PRESENCE/SOS mount hydrate；以及 **A11-D2（USER-GATE，今晚雙機 clean install 實測回填）**。

## [2026-06-21] A11-debug-3 DONE — 關閉 Android 備份修 secure_storage BAD_DECRYPT（不碰 crypto/format）

- repo/commit: IgniRelay @ `49b5f9e`
- 執行者: Claude（Owner 授權 session）；A11 白天 debug。
- **修的問題＝BAD_DECRYPT / backup-restore risk**: 正式 manifest `<application>` 原本**沒有** `android:allowBackup`→預設 `true`。Android 雲端自動備份把 `flutter_secure_storage` 密文備走，還原到 Keystore key 不同的裝置後舊密文解不開（BAD_DECRYPT），導致身份/場域 secret 讀取炸掉啟動流程（`IdentityManager.initialize` 在 `main.dart` 階段 1「失敗＝無法啟動」）。read-only 確認三處 secure-storage read 皆未對 `read()` 拋例外設防（既有 try/catch 只包 base64 decode，不含 read 本身）。
- 修（兩層）:
  - **根因**: manifest 加 `android:allowBackup="false"`——停掉密文被備份/還原的途徑。manifest merge 乾淨（無須 `tools:replace`）。
  - **讀取側 self-heal**（涵蓋 allowBackup 擋不到的殘餘途徑：Android 12+ D2D 轉移、OS 憑證/生物辨識變更導致的 Keystore key 失效）。窄範圍——只攔持久化 read、有 log、不吞其他錯誤：
    - `IdentityManager.initialize`: read 拋 → 視為無金鑰 → `_generateAndSave()` 重生並覆寫壞密文（＝clean install 狀態，非崩潰）。read/write 改走共用 `SecureKvStore` seam 讓恢復路徑可單測（`debugSecureStoreOverride`，比照 `DatabaseHelper.testDatabasePathOverride`）。
    - `AnonIdentityService.getOrCreate`: read 拋 → 視為不存在 → 重鑄 anon id（覆寫）。
    - `FieldSessionStore.secretFor`: read 拋 → 回 `null`（既有「secret missing」契約；**不自動刪**——`leave()` 才是顯式移除路徑，對暫時性平台錯誤刪 secret 過度破壞）。
- 範圍: **不碰** crypto / 金鑰・field secret 格式 / QR / membership model；**不碰** wire/proto/canonical/corpus；**不碰** HAZARD / SOS / LastSeen hydrate / runbook；**無新依賴**。未加 `dataExtractionRules` 資源——self-heal 已涵蓋 D2D/Keystore 失效，allowBackup=false 已擋雲端備份，分開的 extraction rule 非必要。
- 變更檔（8，+226/−11）: manifest；`identity_manager.dart` / `anon_identity.dart` / `field_session_store.dart`（catch + seam）；`anon_identity_test.dart` / `field_session_store_test.dart`（+1 each）；新 `test/android/manifest_backup_test.dart`、`test/crypto/identity_manager_test.dart`。
- test（+4）: manifest guard（斷言 `allowBackup="false"`、永不 `="true"`）；identity_manager / anon_identity / field_session_store 各模擬 `read` 拋 BAD_DECRYPT（`PlatformException`）→ **不 crash → 回到可恢復狀態**（重生金鑰已持久化 / 重鑄 id / 回 null）。
- gates（G17 逐字，全綠）:
  - `dart run tool/check_layers.dart --strict` → `ok — no boundary violations`，exit 0
  - `flutter analyze --no-fatal-infos --no-fatal-warnings` → `2 issues`（皆 baseline battery_optimization_guide.dart:27/249，0 新），exit 0
  - `flutter test --exclude-tags golden` → `All tests passed! +730 ~3`（+4），exit 0
  - `flutter test test/conformance/wire_conformance_corpus_test.dart` → `+21`（未動 wire），exit 0
  - GATE-KOTLIN-BUILD `:app:assembleDebugAndroidTest` → **BUILD SUCCESSFUL**（manifest merger 接受 allowBackup=false），exit 0
- deviations: 無。**未宣稱 A11 通過。**
- **今晚 A11 驗收仍須 clean install / clear app data 後重跑 USER-GATE**：allowBackup=false 只防後續備份；已存在於裝置的壞密文需清資料才會被乾淨重生路徑取代（且承接 A11-debug-2-fix 的髒 DB 清除需求）。
- next: **A11-debug** 籃子剩項——LastSeen PRESENCE/SOS mount hydrate；以及 **A11-D2（USER-GATE，今晚雙機 clean install 實測回填，AI 可代跑 ADB 但不得代填/宣稱通過）**。

## [2026-06-21] A11-debug-4-fix DONE — LastSeen 從 read-model mount hydrate（+順帶修收端 SOS 座標欄位）

- repo/commit: IgniRelay @ `287710f`
- 執行者: Claude（Owner 授權 session）；A11 白天 debug。經 A11-debug-4 read-only 界定確認 read-model 充足、不需動 DB/projector/wire。
- **修的問題＝LastSeen mount hydrate**: `LastSeenScreen` 原本只訂閱 live broadcast typed streams，這些 stream 不重播 → 重啟後（或分頁在事件抵達後才 mount）位置頁空白到下一筆 live event 才出現。HazardCard 早已用 `recentHazards` mount backfill 解過，LastSeen 漏補。
- **順帶修正（Owner 核准併入，選項 1）＝收端 SOS 座標欄位讀錯**: `Event_Logs` **無 `lat`/`lng` 欄位**，只有 `received_lat`/`received_lng`（`ingestVerifiedEvent` / v2 projector 寫收到事件座標處）。但 EventStream SOS dispatch 讀 `row['lat']`/`row['lng']`→ 恆 null → 收到的 SOS 永遠掉座標、收方顯示「（無座標）」即使送端有 GPS。**這是 A11-debug-1 收端那半**（debug-1-fix 只修送端）。live dispatch 與 `recentSos` 現都改讀 `received_lat`/`received_lng`。**純 receive/read 側，未碰 SOS/SAFE send path。**
- 修:
  - `EventStream` +4 backfill API（讀既有 Event_Logs 列，無 DB/projector/wire 變更）: `recentPresence`/`recentCheckpoints`（queryByType 9001/9003→snapshot 解碼）、`recentSos`（queryRecentSos：requestBroadcast urgency≥2、24h）、`recentSosResolutions`（queryByType 9002→author key）。列→typed 映射抽成 `_sosFromRow`/`_sosResolvedFromRow`，**live 與 backfill 共用同一份**（不分裂）。
  - `LastSeenScreen`: initState 起一次性 `_hydrate()`（backfill seam 預設接 EventStream 查詢、可注入測試）；eventId 去重（`_seenPresence`/`_seenCheckpoint`/`_seenSos`，live+backfill 共用）；**SOS+SAFE 合成單一 timestamp timeline 由舊到新 replay**，author-LWW 與 live 到達序一致（old SOS+later SAFE→清除；old SAFE+later SOS→顯示）；best-effort，backfill 失敗不影響 live。
- 範圍: UI 只透過 EventStream（不直接碰 DB/EventStore，check_layers 綠）；**不碰** HAZARD/secure_storage/SOS·SAFE send path/wire/proto/canonical/corpus/runbook；無新依賴。
- 變更檔（4，+476/−26）: `event_stream.dart`（backfill API + 共用 row→typed helper + dispatch 改讀 received_lat/lng）/ `last_seen_screen.dart`（backfill seam + hydrate + 去重 + timeline）/ `event_stream_test.dart` / `last_seen_screen_test.dart`。
- test（+8）: EventStream `recentSos` 座標來自 received_lat/lng、`recentPresence`、`recentSosResolutions`；LastSeen mount backfill PRESENCE、SOS 帶座標、SOS→SAFE / SAFE→SOS timeline 排序、backfill+live 同 eventId 去重。
- gates（G17 逐字，全綠）:
  - `dart run tool/check_layers.dart --strict` → `ok — no boundary violations`，exit 0
  - `flutter analyze --no-fatal-infos --no-fatal-warnings` → `2 issues`（皆 baseline battery_optimization_guide.dart:27/249，0 新），exit 0
  - `flutter test --exclude-tags golden` → `All tests passed! +738 ~3`（+8），exit 0
  - `flutter test test/conformance/wire_conformance_corpus_test.dart` → `+21`（未動 wire），exit 0
  - GATE-KOTLIN-BUILD `:app:assembleDebugAndroidTest` → **BUILD SUCCESSFUL**（未碰 native，仍實跑），exit 0
- deviations: **A11-debug-4-fix 順帶修正 receive-side SOS 座標欄位讀錯（`lat`/`lng` → `received_lat`/`received_lng`），不碰 send path。** **未宣稱 A11 通過。**
- **今晚 A11 驗收仍須 clean install / clear app data 後重跑 USER-GATE**（承接 debug-2-fix 髒 DB + debug-3 壞密文清除需求）。
- next: 白天 debug 實作面收斂；接著更新晚上 runbook/ADB 前置腳本，再 **A11-D2（USER-GATE，Owner 雙機 clean install 實測回填 13 步，AI 可代跑 ADB 但不得代填/宣稱通過）** → A12 契約凍結。

## [2026-06-21] A11-debug-4-polish DONE — LastSeen backfill 改 oldest-first replay

- repo/commit: IgniRelay @ `71f9e7f`
- 執行者: Claude（Owner 授權 session）；A11 白天 debug 收尾 polish。
- 問題（Owner review 抓到，非 blocker）: `EventStore.queryByType` 回 newest-first（`hlc_timestamp DESC`），但 `LastSeenScreen._cap()` 從 list **開頭**刪（對 live 正確，live oldest→newest 追加）。`_hydrate` 原本照 backfill newest-first 直接 replay PRESENCE/CHECKPOINT → 同一 anon 超過 `_maxObsPerSubject=12` 筆時，**最新一筆會被 cap 刪掉**，位置頁顯示比 read-model 實際持有的更舊。
- 修: `_hydrate` 對 PRESENCE/CHECKPOINT backfill 先依 `observedAt` 升序排序再 replay（排在 authoritative 觀測時間，而非 DESC 列序＝hlc_timestamp）。SOS/SAFE timeline 本就 timestamp 排序、不動；live handler 與 EventStream API 不動。
- 範圍: **不碰** HAZARD/secure_storage/SOS·SAFE send path/wire/proto/DB/runbook；無新依賴；只動 2 檔（last_seen_screen + test）+47/−2。
- test（+1）: 同一 anon 餵 13 筆 backfill（newest-first），最終保留**最新**那筆（distinctive 座標 25.00012）、而非被 cap 刪掉降級成 25.00011。CHECKPOINT 共用同一 `_byAnon`/`_cap` 路徑，由此測涵蓋。
- gates（G17 逐字，全綠）:
  - `dart run tool/check_layers.dart --strict` → `ok`，exit 0
  - `flutter analyze --no-fatal-infos --no-fatal-warnings` → `2 issues`（baseline，0 新），exit 0
  - `flutter test --exclude-tags golden` → `All tests passed! +739 ~3`（+1），exit 0
  - `flutter test test/conformance/wire_conformance_corpus_test.dart` → `+21`，exit 0
  - GATE-KOTLIN-BUILD `:app:assembleDebugAndroidTest` → **BUILD SUCCESSFUL**，exit 0
- deviations: 無。**未宣稱 A11 通過。**
- next: **白天 debug 實作面收斂完成**。下一步＝更新晚上 A11 **runbook / ADB 前置腳本**，再 **A11-D2（USER-GATE，Owner 雙機 clean install 實測回填 13 步，AI 可代跑 ADB 但不得代填/宣稱通過）** → A12 契約凍結。**今晚仍須 clean install / clear app data**（承接 debug-2/3 髒 DB + 壞密文清除）。

## [2026-06-21] A11-runbook-prep DONE — runbook 納入 debug-1~4 重測點 + clean install + 前置 helper

- repo/commit: IgniRelay @ `52568a6`
- 執行者: Claude（Owner 授權 session）；**docs/script only，未碰 app code / wire / tests**（故不適用 G17 Flutter gates；helper 已 PowerShell Parser 解析 OK 並 dot-source 載入 11 個函式）。
- 目標: 把今天 A11-debug-1~4 修正後的實機驗收前置與重測重點寫清楚，讓晚上兩台手機接上後可直接跑、不現場摸索。
- 改 runbook（`docs/ACCEPTANCE_A11_TWO_PHONE_SCRIPT.md`）:
  - 新 §0.2「今晚必做：clean install + A11-debug-1~4 重測重點」——**強制兩台 clean install / clear app data**，明寫理由：debug-2-fix 舊 DB 可能有 HAZARD v1+v2 雙列髒資料、debug-3 `allowBackup=false` 清不掉**已被還原**的舊密文/舊 app data、舊 bare 包殘留；附 debug→重測點→步驟對照表。
  - §2 安裝流程改**強制 clean install**：卸載 3 個 id（`network.ignirelay`／`.field`／`.field.test`）→ `adb install` 新 APK → 查包名；**移除 `install -r` 蓋舊資料 anti-pattern**（見 [[secure-storage-backup-decrypt]]）。測前/測後包名皆不得有 bare `network.ignirelay`。
  - 重測點寫入步驟：Step 1（secure_storage BAD_DECRYPT 不卡死啟動、`regenerating/minting fresh` 屬自癒非 FAIL）、Step 7（**收方 SOS 應顯座標、非「（無座標）」**＝debug-1-fix+debug-4-fix）、Step 9（HAZARD v2-only：**發送端不自列自己的 HAZARD 屬刻意非 FAIL**，看 A 接收端）、Step 10（重啟**先看位置頁 read-model 回填**＝debug-4-fix mount hydrate＋v2-only 不重複＋不卡死，A/B 兩台都重啟）；彙總表 7/10 同步。
- 新 helper（`ignirelay_app/scripts/a11_devicetest.ps1`，比照 `scripts/gen_proto.ps1` 慣例）: dot-source 提供 `New-A11Evidence`/`Reset-A11Device`（卸 3 包→裝→查包名）/`Get-A11Packages`/`Clear-A11Logcat`/`Start-A11App`/`Get-A11Shot`/`Save-A11Logcat` 等；**只做機械前置與證據擷取、絕不判 PASS/FAIL**（程式內無任何 PASS/FAIL 輸出，USER-GATE 紅線）。
- 變更檔（2，+270/−25）: `docs/ACCEPTANCE_A11_TWO_PHONE_SCRIPT.md`（runbook）+ 新 `ignirelay_app/scripts/a11_devicetest.ps1`。**未 stage** 未追蹤的 `docs/A11_DEVICE_TEST_RESULTS_2026-06-21.md`／`docs/KNOWN_ISSUE_secure_storage_backup_decrypt.md`／`tmp/`（依 Owner 指示）。
- deviations: 無。**docs/script only；A11 仍未通過；晚上仍須 Owner 手動回填 PASS/FAIL。**
- next: **白天 debug + runbook 前置全部就緒**。只剩 **A11-D2 USER-GATE**（今晚 Owner 接兩台手機、先 clean install，照 runbook 13 步逐項回填；AI 可代跑 ADB/Gradle/截圖/logcat 但不得代填/宣稱通過）→ 通過後才 A12 契約凍結。

## [2026-06-21] UI-H3-evidence-polish DONE — H3 large-text 測試補齊 1.15/1.30/1.45/2.0 字級證據（test-only）
- repo/commit: IgniRelay @ `a8a0f3d`
- 執行者: Claude（Owner 本 session 接任主理）。Owner 補核 H3 + H3-polish 實際 diff 後判定：Required Screens 已合齊，但 `docs/UI_H_I18N_A11Y_PLAN.md` §UI-H3 §Tests 明列測試應涵蓋 `1.15 / 1.30 / 1.45 / 2.0`，而 H3/H3-polish 壓測只斷言 1.45 + 2.0 → 文件 DoD 證據缺口。本刀只補測、不改 UI。
- 範圍: **test-only。未改 layout / ARB / 文案 / 功能 / wire / proto / DB / native。未全域 cap text scale。** 把每個既有 H3 large-text 壓測迴圈由 `[1.45, 2.0]` 擴成 `[1.15, 1.30, 1.45, 2.0]`；兩個原單一 2.0 的 H3-polish 測試（joined AppShell bottom nav + global SOS、MyTab settings）改為四字級迴圈。
- 改動（5 test 檔，**0 新增 test case**——僅放寬迴圈內斷言）:
  - `app_shell_test`：安全 / no-field 壓測迴圈 → 四字級；joined AppShell（五分頁 label + 每 tab global SOS 可達 + 不爆版）與 MyTab settings 由單 2.0 pump 改四字級迴圈；
  - `field_screen_test` / `sos_screen_test` / `last_seen_screen_test` / `preview_screen_test`：H3 壓測迴圈 → 四字級。
- 修測試 harness 一處（非產品碼）：joined AppShell 迴圈每字級結束 pump `MaterialApp(home: SizedBox.shrink())` tear down——unkeyed AppShell 跨 re-pump 會復用 State、保留上一輪選到的分頁，導致 MyTab onstage「我的」標題與底部 nav label 撞號（`find.text` 預設略過 IndexedStack offstage 子樹，故原單次 pump 未觸發）。
- DoD（對齊 §UI-H3 §Tests）: ✅ 1.15 / 1.30 / 1.45 / 2.0 四字級皆有明確 no-overflow 斷言；✅ no-field + global SOS reachability 在 composite 仍綠；✅ test-case 數未變（727 testWidgets/test 宣告 HEAD==now）。
- gates（G17 全綠）:
  - `dart run tool/check_layers.dart --strict` → `ok — no boundary violations`，exit 0
  - `flutter analyze --no-fatal-infos --no-fatal-warnings` → `2 issues`（皆 baseline battery_optimization_guide.dart:27/249，0 新），exit 0
  - `flutter test --exclude-tags golden` → `All tests passed! +739 ~3`（**0 新 case**，test-only 放寬斷言）
  - `flutter test test/conformance/wire_conformance_corpus_test.dart` → `+21`（未動 wire），exit 0
  - GATE-KOTLIN-BUILD `:app:assembleDebugAndroidTest` → **BUILD SUCCESSFUL in 5s**，exit 0
- deviations: **更正先前記錄**——可重現全套組總數為 **739**（非 UI-H3-polish 條目所記的 715；該 `+715` 係 runtime 計數器一時誤讀，已用 `git stash` 還原 HEAD 實測 739 對照確認）。**未宣稱 UI-H DONE、未宣稱 A11 passed。**
- next: **UI-H4 legacy cleanup**（待 Owner 明確發落；移除確認 dead 的舊 `map*`/`hazardDialog*` 等 ARB key 與死碼，先 grep/usage 證零引用再刪）→ A11-D2 USER-GATE（Owner 雙機回填 13 步，AI 可代跑 ADB 不得代填/宣稱通過）。

## [2026-06-21] A11-live-fix DONE — SosScreen 從 read-model mount backfill（收端 only；budget 假說經實機推翻）
- repo/commit: IgniRelay @ `8cf44af`
- 執行者: Claude（Owner 本 session 接任主理）。緣由：GPT 雙機重測 SOS 仍未過；另一 AI 指「SOS 帶座標踩 240 budget → 收方 `over-budget-sos-received` drop」為主因。Owner 要 Claude 獨立讀碼 + 實機雙機測試確認真因（不得為應付而附和）。
- **實機雙機測試（A=94067a07 / B=LNZ5TKY5NRNVQ4K7，證據 `tmp/a11-live/`）關鍵發現**——同一筆 SOS envelope `019EEACE1076…` 串起全鏈：A `Mesh_Trace_Logs` `SENT et=1 pr=1`(SOS_RED) → B `RECV et=1 pr=1 **drop=None** sig=0` → B `Event_Logs` `et=1 urg=3 **lat=22.6412706 lng=120.3095473**` → B 開著 SOS 頁時 live 顯示「附近求救(1) 受困 22.64127,120.30955」。
  - **budget 假說實機推翻** ❌：SOS_RED 在收方 trace 是 `drop=None`，envelope 到達即正常接受、含座標投影。sender/receiver 對同一份 bytes 用同一 240 cap，A 顯示送出代表 sender 已過 → receiver 必過。
  - **根因②（本刀修）= SosScreen 無 mount backfill** ✅：只訂 live broadcast（不重播）；B 切走再切回 SOS 頁顯示「附近求救(0)」而**同刻 Event_Logs 仍有該筆 SOS**——確定性重現昨晚「A 送達/B 看不到」。
  - **根因③（未修，另開診斷線）= BLE 連線反覆斷線重握手**：兩台 trace 每 25–70s 一輪 `et=100`(PROTOCOL_HELLO) SENT/RECV → SOS 撞「無 ready peer」窗口進 outbox queue（A 顯示「已排入佇列(無在線鄰近裝置)」=昨晚第一輪），~20s link 恢復 drain 才送達。非 SOS 專屬、非 budget；根在原生 BLE 連線管理。
- 範圍: **receiver-only UI**。未碰 send path / wire / proto / DB / native / SosController / secure_storage。
- 改動（2 檔）:
  - `lib/ui/screens/sos/sos_screen.dart`：加 4 個測試 seam（`alertSource`/`resolvedSource`/`alertBackfill`/`resolvedBackfill`，預設取 DI `EventStream`）；mount `_hydrate()` 把 `recentSos()`+`recentSosResolutions()` merge 成 timestamp-ordered timeline 並 oldest→newest replay（author-LWW 與 live 一致）；inline listener 抽成 `_onAlert`/`_onResolved`（live 與 backfill 共用）；`_seenSos` eventId dedup + keep-newest-per-author 防 backfill 蓋掉較新 live；新增 `_SosTimelineEvent`。
  - `test/ui/screens/sos_screen_test.dart`：+3（mount 回填顯示已落地 SOS+座標／SAFE-after-SOS→已解除／SOS-after-SAFE→仍 active）。
- DoD: ✅ 確定性重現昨晚並修復；✅ timeline 兩向皆正確；✅ token-clean（無 `Colors.`/hex／無「目前位置」）；✅ `recentSos()`/`recentSosResolutions()` 已存在（A11-debug-4-fix），本刀只接線。
- gates（G17 全綠）:
  - `dart run tool/check_layers.dart --strict` → `ok — no boundary violations`，exit 0
  - `flutter analyze --no-fatal-infos --no-fatal-warnings` → `2 issues`（皆 baseline battery_optimization_guide.dart:27/249，0 新），exit 0
  - `flutter test --exclude-tags golden` → `All tests passed! +742 ~3`（+3 新 SOS backfill case）
  - `flutter test test/conformance/wire_conformance_corpus_test.dart` → `+21`（未動 wire），exit 0
  - GATE-KOTLIN-BUILD `:app:assembleDebugAndroidTest` → **BUILD SUCCESSFUL in 1m 4s**，exit 0
- deviations: **A11 仍未標通過。** 本刀只解根因②（UI backfill）；**根因③（BLE 連線 flapping → SOS queue）尚未修**，是另一條較深的調查線（需實機抓原生 BLE 斷線/重連 log），不在本刀範圍。
- next: Owner 重設雙機 + 重開測試場域 → Claude 實機驗證 backfill 修復（B 停安全頁、A 送 SOS、B 切 SOS 頁應見）→ 接著診斷根因③ BLE flapping。

---

## [2026-06-22] A11-latency-fix DONE — SOS/SAFE 緊急投遞繞過 gossip cooldown（APK #1）
- repo/commit: IgniRelay @ `12b78de`
- 執行者: Claude（主理 AI，Owner 授權 session；經 GPT 派工 task A11-latency-fix，依 `docs/A11_LATENCY_IBLT_STAGED_FIX_PLAN.md` @ ad70000）
- 目標: 修 A11 Step 7「B 端 SOS 告警 ≤10s 出現」的 latency blocker（③）。SOS/SAFE 在無 ready peer 時不再等一般 gossip reconnect/cooldown，觸發 emergency 立即連線。
- 根因（前夜定位）: SOS 走 v2 envelope；無 `isReadyForTraffic` peer → 進 `Outbox_V2` queue，只在下一個連線循環（最壞 ~60s peer cooldown）drain。是延遲、非遺失。
- 修法（與 plan「Required design shape」一致）:
  - `event_publisher_v2_facade.dart`: 新 `EmergencyMeshDelivery` 抽象介面 + 可選注入（建構參數 + `attachEmergencyDelivery`）；`publishStatusUpdate` 算 emergency predicate；`_broadcast` 新增 `emergency`（預設 false），**僅兩個 enqueue 分支**（bridge-null / 無 ready peer）呼叫 `_maybeRequestEmergencyConnect`（best-effort try/catch，不改 outcome、不產生第二個 envelope）。
  - **predicate（鎖死，禁用 `eventType` 判斷）**: `effectivePriority ∈ {sosRed, sosYellow} || safetyState == SAFE`。TRAPPED→sosRed、INJURED→sosYellow、SAFE（雖維持 STATUS priority 仍觸發，SOS 解除須快速傳播）。PRESENCE/CHECKPOINT/HAZARD/ADMIN 不走 publishStatusUpdate → 永遠非緊急（CHECKPOINT 同為 STATUS priority 仍不觸發，證明非「priority==STATUS」）。
  - `ble_manager.dart`: 新 `requestEmergencyConnect()` + 時間盒 `_emergencyConnectUntil`（15s）。窗口內 `_isInCooldown` 回 false（繞過 60s cooldown）＋把近期已同步 peer 重新排入 `_pendingDevices` 立即重連。**窄加法式**：connect→sync→2s→disconnect 主流程與 cooldown 數值不變，窗口過後恢復正常 gossip。判定不需重構 BLE 狀態機，故未觸發 plan 的「停下回報」條款。
  - `ble_emergency_mesh_delivery.dart`（新）: production hook，呼叫 `BleManager().requestEmergencyConnect()`（與 transport 同一 singleton 實例）。
  - `main.dart`: `_startV2Bridge` 內 `attachBridge` 後 `attachEmergencyDelivery(BleEmergencyMeshDelivery())`。
- 邊界遵守: 未碰 wire/proto/canonical/corpus/DB；未動 native（iOS 不受影響）；未以降 `kPeerCooldownSec` 為主修法；PRESENCE/CHECKPOINT/HAZARD cadence 未變；SOS 先 enqueue、emergency 後沿用既有 drain 送**同一** envelope_id（無第二個 envelope）。
- 測試: `event_publisher_v2_facade_test.dart` +7（ready peer 不觸發／TRAPPED 無 peer enqueue 一次+hook 一次+無重複 envelope／INJURED 觸發／SAFE 在 STATUS priority 仍觸發／PRESENCE 不觸發／CHECKPOINT(STATUS) 不觸發／拋例外 hook 不中斷 publish）。
- gates（G17 全綠）:
  - `dart run tool/check_layers.dart --strict` → `ok — no boundary violations`，exit 0
  - `flutter analyze --no-fatal-infos --no-fatal-warnings` → `2 issues`（皆 baseline battery_optimization_guide.dart:27/249；**0 new**），exit 0
  - `flutter test --exclude-tags golden` → `All tests passed! +749 ~3`（+7 emergency case）
  - `flutter test test/conformance/wire_conformance_corpus_test.dart` → `+21`（未動 wire），exit 0
  - GATE-KOTLIN-BUILD `:app:assembleDebugAndroidTest` → **BUILD SUCCESSFUL in 1m 16s**（UP-TO-DATE，未碰 native），exit 0
- APK: `tmp/apk/a11-latency-debug.apk`（`flutter build apk --debug` √ Built；~183MB debug；含 A11-live-fix 以來所有修，無 IBLT 改動）
- device validation: **未跑**。實機 ≤10s 驗證 = Owner 今晚 USER-GATE（AI 不代填、不宣稱通過）。
- deviations: **A11 尚未通過**。本刀僅移除已知 Step 7 latency blocker（③）的程式碼成因；實機是否穩定 ≤10s、以及 13 步 USER-GATE 其餘步驟，仍須含本 APK 實跑 + Owner 逐項填 PASS。IBLT 不在本刀（下一刀）。
- next: Owner 用 `tmp/apk/a11-latency-debug.apk` clean install 雙機 → 測 SOS/SAFE ≤10s（Step 7）。通過後再進 IBLT-fix（APK #2）→ 含修跑完整 A11 USER-GATE。

---

## [2026-06-22] IBLT-fix DONE — iblt-keyhash-v2 peel 契約 + capability gate（APK #2）
- repo/commit: IgniRelay @ `8030211`
- 執行者: Claude（主理 AI，Owner 授權 session；經 GPT 派工 task IBLT-fix，依 `docs/A11_LATENCY_IBLT_STAGED_FIX_PLAN.md` + `docs/IBLT_FIX_PLAN.md`；舊 `IBLT_FIX_HANDOFF.md` 僅歷史參考，未採其「只改 _getIndices」半套）
- 目標: 修跨平台 IBLT fast-path peel bug，凍結前讓契約正確；產出 APK #2。
- 根因（兩處 mismatch，三平台一致）: insert 用 `MurmurHash(eventId.codeUnits)` 進桶、`hashSum=FNV1a(eventId)`；peel 用 CRC bit-extraction 算索引、`verifyChecksum=FNV1a(uint32_le(keySum))` → 兩組 index/checksum 空間不一致 → 真差幾乎永遠 peel 失敗 → 退 Bloom。
- 修法 `iblt-keyhash-v2`（peel 可只從 pure cell 的 keySum 重建一切）:
  - `keyHash=CRC32(eventId)`；`keyBytes=uint32_le(keyHash)`；`checkHash=FNV1a(keyBytes)`；`indices=MurmurHash(keyBytes, seed 0/1/2)%56`。
  - insert/remove **與** peel 都用同一 `keyHash` 推導 index+checksum → 小對稱差確定可 peel，結果＝差集 eventId 的 CRC32 hashes。外形不變（504B、keySum=CRC32 XOR、peel 結果=uint32 hashes），僅內部推導改 → bucket bytes 變 → fixture/corpus 重生。
  - 三平台同刀: `lib/app/mesh/iblt.dart`（oracle）+ `android/.../IBLT.kt` + `ios/Runner/IBLT.swift`，演算法 byte-identical。
- capability gate（混版安全）: self-HELLO 的 `capabilities` 加 `iblt-keyhash-v2`（`main.dart` buildSelfHello）；BleManager 新 `ibltContractCheck`，僅當 peer HELLO 宣告該能力才走 IBLT，否則 Bloom（純函式 `peerSupportsIbltKeyHashV2` 讀 `PeerCapabilityRegistry`，main.dart 接線）。**未加 IBLT payload version byte**（用 capability 協商取代，不動封包外形）。**mixed old/new peers → Bloom fallback**，絕不跨契約 peel；Bloom 為正確性安全網。
  - **誠實 caveat（非 blocker）**: IBLT 嘗試發生在 `_nordicConnectAndSync` 早期，而 v2 HELLO 能力是非同步落 registry、且每次 connect-sync-disconnect 後 registry 會 evict；故現行連線生命週期下 fast path **多半不會觸發 → 走 Bloom**（正確性不受影響，這是 ③ 連線時機問題，非本刀範圍）。本刀交付＝契約正確 + 跨平台 parity + corpus 凍結就緒 + 混版安全，**非**提高 runtime 觸發率。
- 邊界遵守: 未動 A11-latency-fix 的 emergency delivery；未動 envelope/proto/canonical/DB；wire 僅 IBLT bucket bytes + HELLO capabilities（預期內、已重生 corpus）。
- 測試:
  - `iblt_test.dart` +3（小對稱差 peel＝CRC32 hashes／toBytes→fromBytes→subtract→peel round-trip／全 shared 無假差）。
  - `iblt_swift_parity_test.dart` +1（peel golden vector `peel_symmetric_diff_3` 對 live Dart）。
  - 新 `iblt_capability_gate_test.dart` +5（宣告→允許／空能力→Bloom／他能力→Bloom／未知 peer→Bloom／pending 無 HELLO→Bloom）。
  - fixture `iblt_swift_parity_vectors.json`（7→8 cases，含 peel 黃金向量）+ corpus `wire_conformance_v1.json`（IBLT bucket bytes 重生）皆由 generator 自 Dart oracle 重產。
- gates（G17 全綠）:
  - `dart run tool/check_layers.dart --strict` → `ok — no boundary violations`，exit 0
  - `flutter analyze --no-fatal-infos --no-fatal-warnings` → `2 issues`（皆 baseline battery_optimization_guide.dart:27/249；**0 new**），exit 0
  - `flutter test --exclude-tags golden` → `All tests passed! +758 ~3`（較 A11-latency-fix +9）
  - `flutter test test/mesh/iblt_test.dart` → +14 全綠
  - `flutter test test/mesh/iblt_swift_parity_test.dart` → +8 全綠
  - `flutter test test/conformance/wire_conformance_corpus_test.dart` → +21 全綠（corpus_revision=`v0.3-iblt-keyhash-v2-1` 已同步測試斷言）
  - GATE-KOTLIN-BUILD `:app:assembleDebugAndroidTest` → **BUILD SUCCESSFUL in 12s**（IBLT.kt 重新編譯，12 tasks executed），exit 0
- iOS: Swift source（IBLT.swift）+ Swift parity 測試（IBLTParityTests 加 peel golden vector）已對齊新契約，但 **XCTest 未跑，原因：no macOS available**。Android 儀器測試（WireConformanceInstrumentationTest 讀 corpus）已對新 IBLT.kt 編譯通過，device-run 同前遞延。
- APK: `tmp/apk/a11-latency-iblt-debug.apk`（含 APK #1 全部 + IBLT-fix；`flutter build apk --debug` √ Built；~183MB debug）
- device validation: **未跑**。
- deviations: **A11 尚未通過**。晚上分段測：先 APK #1（`a11-latency-debug.apk`）確認 SOS/SAFE ≤10s；再 APK #2（本顆）確認 IBLT 未回歸 latency/sync；兩顆 OK 才用 APK #2 clean install 跑完整 13 步 USER-GATE（Owner 逐項填 PASS，AI 不代填）。
- next: Owner 晚上分段實機測 → 通過後 A12 契約凍結（corpus 已就緒）。

---

## [2026-06-22] UI-H4 DONE — legacy UI / l10n cleanup（UI-H 收尾，但 A11 尚未通過）
- repo/commit: IgniRelay @ `2b80155`（branch `rollback/ad70000`）
- 執行者: Claude（主理 AI，Owner 授權；經 GPT 派工，依 `docs/UI_H_I18N_A11Y_PLAN.md` §UI-H4 + `docs/UI_H_I18N_A11Y_AUDIT.md` §3/§5）
- 目標: UI-H 最後一刀——刪除已不在正式路徑的 legacy 畫面與其 dead l10n keys，收乾淨 ARB / 分析基線。Owner 裁定**直接刪，不保留 debug-only**。
- 刪除檔（皆無 live import；A11-preflight-fix 已 de-wire，僅 main.dart 歷史註解提及）:
  - `lib/ui/secondary/onboarding_screen.dart`（231 行）
  - `lib/ui/secondary/battery_optimization_guide.dart`（471 行）
  - ⇒ `lib/ui/secondary/` 整個目錄消失。
- ARB 修剪（`app_zh.arb` + `app_en.arb`，全程維持 parity）:
  - **1351 → 284 message keys**（每檔刪 1067 dead key + 對應 @metadata；兩檔各刪 1152 entries）。
  - keep-set 為 **compiler-driven**，**不採** UI-H0 audit「main* + tierLabel* 才 live」那句（該句 pre-UI-H2，H2 已把正式 UI 在地化 → 早已過時）：一個 key 存活 iff `.<keyName>` token 出現在任一已編譯 `lib/` 或 `test/` 原始碼（排除 generated 與本刀刪的兩畫面）。此法為**安全超集**（連字串/註解都算），故**不可能誤刪 live getter**，僅可能略為保留；最終以 analyze + build 為準。
  - 刪除群組: onboarding(14)、battery(46)、map(139)、hazardDialog(其餘 hazard 群)、supply(388)、station(78)、medical(61)、chat(55)、profile(43)、match(34)、handoff(29)、survival(18)、community(18)、requests/req/neg(各 18-19)、supplies(16)、triage(9)、舊 location(14)/nav(17)/tab(5)/common(5)、itemCondition(3)、mainTab*（舊 map 時代 SOS/match 通知 handler）。
  - 保留 284 = 全部 UI-H2/H2b/H2c 正式 UI keys（field/sos/safety/preview/checkpoint/hazardCard/admin/myField/shellTab/lastSeen/radar/gps/motion/comms/settings/…）+ main*(7) + tierLabel*(4，經 `tier_manager.dart getTierLabel(S l)` 參數存取，故初版 alias 抓漏、改 exact-token 後正確保留)。
- generated l10n: `flutter gen-l10n` 重生 `app_localizations.dart` / `_en` / `_zh`（三檔大幅縮小）。
- 啟動流程: **未碰**。main.dart 三處說明「已移除 onboarding gate」的註解屬語意仍真的歷史文件（plan 明列保留此類註解、且 `rg OnboardingScreen` 命中註解為預期），故保留不動。
- 邊界遵守: 未碰 wire/proto/GATT/crypto/DB/corpus；未碰 A11 latency/IBLT/APK code；**未** stage `.gitignore`（M）或任何 untracked docs/tmp/HANDOFF 雜檔（僅 stage 7 個 H4 檔）。
- gates（G17 全綠）:
  - `dart run tool/check_layers.dart --strict` → `ok — no boundary violations`，exit 0
  - `flutter analyze --no-fatal-infos --no-fatal-warnings` → **No issues found!**（**baseline 2 個 use_build_context_synchronously info → 0**，原寄生於已刪的 battery_optimization_guide.dart:27/249），exit 0
  - `flutter test --exclude-tags golden` → `All tests passed! +758 ~3`（含 `first_run_routing_test` 仍綠）
  - `flutter test test/conformance/wire_conformance_corpus_test.dart` → `+21`（未動 wire），exit 0
  - GATE-KOTLIN-BUILD `:app:assembleDebugAndroidTest` → **BUILD SUCCESSFUL in 4s**（UP-TO-DATE，未碰 native），exit 0
- 額外檢查: ARB zh/en key parity 一致（284=284，集合相同）；`grep` 確認無 onboarding_screen/battery_optimization_guide live import；無 OnboardingScreen/BatteryOptimizationGuide production construction（僅註解）。
- deviations: **A11 尚未通過**。本刀純介面層垃圾回收，與 A11 實機驗收無關。
- 結論: **UI-H 全系列 DONE**（H0 audit / H1 設定 / H2a-c 正式 UI i18n / H3 large-text QA + evidence-polish / H4 legacy cleanup）。**但 A11 仍未通過** —— 下一棒回到今晚 APK #1（`a11-latency-debug.apk`）/ APK #2（`a11-latency-iblt-debug.apk`）分段實機 → 完整 13 步 A11-D2 USER-GATE（Owner 逐項填 PASS，AI 不代填、不宣稱通過）→ A12 契約凍結。

---

## [2026-06-23] A11-D2 USER-GATE COMPLETE — A11 驗收通過

- repo/commit: A11-fix @ `99a3eda`（測試修正）；本 STATUS 為其 docs commit
- 執行者: Claude（主理 AI）；Owner 2026-06-23 授權代跑 + 回填可機驗步驟（誠實標註驗證者，AI 不代填 Owner 自驗項）
- 範圍: APK#2 `tmp/apk/a11-latency-iblt-debug.apk`（@ `8030211`）雙機實機驗收
  A=25102PCBEG(myron/owner) / B=2311DRK48G(duchamp/participant)
- 方法（AI 代跑）: adb input 驅動 UI（`motionevent` 長按 / `tap`，座標取自 uiautomator bounds，無讀截圖入context）；
  logcat `-G 16M` + `--pid` 抓 `[BLE-DBG]`；`adb exec-out run-as cat` 拉 `resqmesh_local.db`(+wal/shm) → python 查
  `Mesh_Trace_Logs`/`Event_Logs`（device 無 sqlite3、`run-as >/sdcard` 遭 SELinux 擋 0 bytes）；helper 留 `tmp/a11*.ps1`
  /`tmp/a11_*.py`、證據 `tmp/a11-evidence/`。
- 結果: 13 步全 PASS（明細見 docs/ACCEPTANCE_A11_TWO_PHONE_SCRIPT.md §5 驗證者標註）
  - IBLT 正常、無回歸: capability gate 5/5 雙向 cleanly skip→Bloom、零 error（DECODE_ERR/PEEL failed/sync failed=0）；
    fast path 休眠（連線當下 registry 未見對方 `iblt-keyhash-v2` 能力 → Bloom，已知 caveat，A11 不要求其觸發、Bloom 同步正常）；
    較 APK#1 少一個每輪 ~8s IBLT timeout 空轉。
  - Step 7 SOS_RED: 斷線 cooldown 窗 `EMERGENCY_CONNECT → cooldown bypassed 15s` → B 收同 envelope ≈3.8s + 帶座標
    `recv=(22.64129,120.30949)`（對照修前 ~20s）。Step 8 SAFE（et=9002 解除）/ Step 9 HAZARD（B→A et=50/alert、backfill
    顯示、帶座標、發送端不自列）/ Step 10 重啟 read-model 回填+dedup+不卡死 皆 AI 機驗。
  - Step 1/2/3/4/5/6靜置 AI 機驗；Step 6 移動態 / 11 / 13 Owner 實機自驗。
  - Step 12 `:app:connectedDebugAndroidTest` → BUILD SUCCESSFUL、Finished 5 tests（device 25102PCBEG）；gate 後無 bare `network.ignirelay`。
- 過程修正（A11-fix @ `99a3eda`）: connectedTest 揪出 IBLT-fix 漏改的 Kotlin `WireConformanceInstrumentationTest` + Swift
  `WireConformanceTests` 仍斷言舊 `corpus_revision v0.3-phase0b-4-6-1`（IBLT-fix gate 只跑 `assembleDebugAndroidTest`、未實跑
  `connectedDebugAndroidTest` 故漏網）→ 修為 `v0.3-iblt-keyhash-v2-1`，重跑 5/5 綠。變更僅 2 個測試斷言檔。
- deviations:
  - IBLT fast path runtime 休眠（gate skip→Bloom）＝已知 caveat、非回歸、不擋 A11（A11 不要求 fast path 觸發）。
  - `connectedAndroidTest` 生命週期跑後卸載 app+test 包；A 期間被卸載一次（資料清空），已 `adb install` APK#2 復原（全新、無場域）。
  - Step 6 移動態 / 11 / 13 為 Owner 實機自驗（AI 未代為觀測），已誠實標註、非 AI 機驗。
  - iOS Swift `WireConformanceTests` 源碼已對齊新 revision 但未跑（no macOS available）。
- next: A12 契約凍結

---

## [2026-06-23] A12 — App↔Node 契約凍結 IMPLEMENTATION DONE（待 D4 Owner 簽核）

- repo/commit: A12 code+spec+corpus @ `d3e820b`；本 STATUS 為其 docs commit
- 執行者: Claude（主理 AI，A12 屬 §0.4 主理 AI 職權）；Owner 2026-06-23 授權執行
- 任務: MASTER_EXECUTION_PLAN §5 A12（交付 Stage B 的鑰匙）。全 additive，未改既有 wire/canonical/field-event 位元組
- D1 spec（normative，無 TBD）: 新 `docs/specs/app_node_gatt_v1.md` —— GATT service/char UUID 逐字收錄自
  `IgniRelayConstants`（不發明新 UUID）；node HELLO additive 欄位 10-13（node_id/node_lat_1e7/node_lng_1e7
  /install_accuracy_m，手機 decode 已 `default:skipValue` 容忍未知 → 舊手機不破）；NODE_RECEIPT=105 control frame；
  chunking 引 native_transport_v1 §4；PHASE3 §7.2 三段收據模型（NODE_RECEIPT 只承諾第一段）
- D2 EventType 105:
  - `event_envelope_v2.dart`: `EventTypeV2.nodeReceipt=105` + `maxHopsDefault(105)=0`（link-local 不轉送）+
    `isKnown(105)`；新 `NodeReceiptData {1 ref_envelope_id b16, 2 status u8, 3 queue_depth u32}` + `NodeReceiptStatus`
  - `priority_matrix_v2.dart`: 105 → 僅 NORMAL，其餘 drop（比照 HELLO/trace）
  - 收端（App 只收不送、**不投影 Event_Logs**）: `EventDecoder.decodeNodeReceipt`+`NodeReceipt` model；
    `V2InboundProjector` 攔 105 解碼 → `nodeReceipts` 流（非 `_ingest`）；`EventStream.nodeReceipts` 轉發；
    `main.dart` 接線 projector→EventStream；`debug_shell`「Node 收據（已送達節點）」卡
  - `envelope_v2_spec §4` `EVENT_TYPE_NODE_RECEIPT=105`（reserved 改 106..129）+ §6 matrix 列 + §11 max-hops 列
  - control frame §21.7: zero field_id、無 field_mac、dispatcher 豁免（`isControlEventType` 範圍 100-129 自動涵蓋 105）
- D5 corpus: generator 增 control NODE_RECEIPT envelope 樣本（zero field_id、無 mac、maxHops 0、真 NodeReceiptData、
  Ed25519 簽）；`corpus_revision` `v0.3-iblt-keyhash-v2-1` → `v0.3-a12-node-gatt-1`；Kotlin+Swift WireConformance
  斷言同刀更新；新增 `node_receipt_contract` note
- gates:
  - `check_layers --strict` → ok
  - `flutter analyze` → 0 issues（0 new）
  - `flutter test --exclude-tags golden` → +767 ~3 全綠（含 A12 新測試：NodeReceiptData round-trip/default/skip、
    EventType 105 maxHops/isKnown、HELLO 容忍 node 欄位、matrix 105、projector 105 surfaces+不投影）
  - `flutter test test/conformance/wire_conformance_corpus_test.dart` → +21（新 NODE_RECEIPT 樣本 141B control sig-input 簽章驗證 + determinism gate）
  - KOTLIN `:app:assembleDebugAndroidTest` → BUILD SUCCESSFUL
- deviations:
  - `connectedDebugAndroidTest`（on-device）本回**未跑**：兩台未接。assemble 綠 + Kotlin/Swift revision 斷言已更新
    匹配新 corpus + Dart conformance 於同一 corpus 綠 → 高信心；on-device 實跑留待裝置可用 / Stage A Exit。
  - iOS Swift `WireConformanceTests` revision 已對齊但未跑 XCTest（no macOS available）。
  - ProtocolHelloData node 欄位 10-13 僅 spec 記為 node 端 additive（手機 decode 容忍未知，已測），未在 Dart class
    新增 encode/decode（手機非 node、不送 node HELLO；避免投機欄位 + corpus churn）。
- D4: **Owner 契約簽核待辦**（`A12 contract sign-off: <date>`）；§5.13 Stage A Exit（`STAGE-A-EXIT: PASS`）亦待 Owner。
- next: D4 Owner 簽核 → Stage A Exit → Stage B（B1 契約包凍結，前置 A12）

---

## [2026-06-23] A12 contract sign-off + STAGE-A-EXIT: PASS（Owner 授權代記）

A12 contract sign-off: 2026-06-23
STAGE-A-EXIT: PASS

- 簽核者: Owner（simon），2026-06-23 明確授權 AI 代記 D4 契約簽核 + Stage A Exit（誠實標註）；A12 屬 §0.4 主理 AI 職權，Owner 選擇接受 deferred connectedTest 直接收尾
- A12 契約凍結（`docs/specs/app_node_gatt_v1.md` + EventType 105 + NodeReceiptData + HELLO node 欄位 10-13 + corpus `v0.3-a12-node-gatt-1`）= **FROZEN**（附錄 B / G6：此後改動須 Owner 明確核准 → 版本 bump）。實作 @ `d3e820b`。
- §5.13 Stage A Exit 四條：
  1. A0–A10/A10b/UI-F/UI-G/A12 全 DONE（含證據）；A11 Owner 回填全過（@ `8cd9f1d`）✅
  2. GATE-LAYERS/ANALYZE/TEST/PARITY/CONF-DART/KOTLIN-BUILD 於 A12 commit 連跑全綠（check_layers ok·analyze 0·test +767~3〔含 iblt parity〕·Dart conformance +21·KOTLIN assembleDebugAndroidTest BUILD SUCCESSFUL）✅
  3. `grep -rn "_todoWire|TODO_CONTRACT|kDebugFieldJoinSecretHex" lib/` → 0 行 ✅
