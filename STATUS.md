...
## [2026-06-22] hook-fix DONE — pre-commit 改 commit-msg (msg 在 pre-commit 跑時還未寫入)
- repo/commit: IgniRelay @ d87871c
- 修正: 原本只用 pre-commit hook 抓 staged，發現 git commit -m 時 pre-commit 跑在訊息寫入前，導致 STATUS commit 無法正確識別（曾被誤擋）。改用 commit-msg hook 取代——它跑在訊息已寫入後，staged 仍可查，覆蓋全部規則。
- 變更: 刪 tool/hooks/pre-commit、新增 tool/hooks/commit-msg（POSIX sh）、更新 tool/install-hooks.ps1 改為多 hook 安裝、更新 .gitattributes LF 規則。
- 驗證: 4 情境實測（功能 commit / 缺 STATUS 擋下 / STATUS commit 放行清 PENDING / 純 docs 放行）全綠。
- next: 同 STATUS.md 主 entry。
