...
## [2026-06-22] workflow-migration DONE — Claude memory 遷移 Kilo + pre-commit hook 強制兩段式 commit
- repo/commit: IgniRelay @ 211248d (功能) / 後續 STATUS commit
- 執行者: Kilo（Owner session 2026-06-22）。緣由：Owner 從 Claude Code IDE 遷移到 Kilo，發現 Claude 的 memory/ 是手抄摘要會舊，要求改成「活源 + 指標」架構並用 git hook 硬性保證 STATUS.md 不漏更新。
- 變更（5 檔，179 insertions）：
  - .kilo/kilo.jsonc：instructions glob 改成只指紅線檔（.kilo/memory/no-fake-coordinate-rule.md）。
  - .kilo/memory/：4 個進度類 memory 檔全刪（master-execution-plan / secure-storage / sos-ble / MEMORY），只留紅線檔。不再維護手抄摘要。
  - tool/hooks/pre-commit（進 git）：POSIX sh 寫的 hook，規則三條——①docs: STATUS entry 開頭必須 staged STATUS.md（否則擋）；②功能 commit（訊息非 docs:）含程式碼改動（lib/test/tool/ignirelay_app/{lib,test,tool,android,ios,pubspec}*）→ 放行並寫 .git/STATUS_PENDING 標記；③若 PENDING 存在且本次非 STATUS commit → 擋下「上一個功能 commit 尚未補 STATUS.md」。
  - tool/install-hooks.ps1（進 git）：一鍵安裝腳本，複製 tool/hooks/pre-commit 到 .git/hooks/pre-commit、強制 LF、清除殘留 PENDING。
  - .gitattributes：新增 tool/hooks/pre-commit text eol=lf（防止 autocrlf 轉 CRLF 破壞 sh）。
- **新流程（hook 強制）**：①功能 commit 放行 + 標記 PENDING → ②補 STATUS.md（append 新 entry + 覆寫檔尾 Current State 段）並 git add STATUS.md → ③git commit -m 'docs: STATUS entry — <任務> DONE @ <hash>' 放行 + 清 PENDING。跳過②則下次 commit 被擋。
- 全域 ~/.config/kilo/AGENTS.md（不進 git）已寫入新流程規矩作軟性提醒備援。
- gates:
  - hook 實測 4 情境（功能+staged→標記 / PENDING+非STATUS→擋 / STATUS+無STATUS.md staged→擋 / STATUS+staged STATUS.md→放行清標記）全綠。
  - 真實 git commit 跑過一次確認 hook 觸發（PENDING 建立訊息從 stderr 出，commit 仍成功）。
  - STATUS.md 中文無亂碼（多次寫入用 .NET UTF-8 no BOM 驗證）。
  - install-hooks.ps1 跑一次成功重裝（需 UTF-8 BOM 否則 PowerShell 5.1 中文 parser error）。
- deviations: **未動 wire/proto/DB/native/pubspec**。未動 docs/MASTER_EXECUTION_PLAN.md（雖多處引用 Claude，但那是 owner 寫給多個 AI 看，不在本次遷移範圍）。未動 .gitignore（工作樹中既存 /tmp/ 改動非本刀產出，留給原作者處理）。
- **未動工作樹中其他 untracked 項目**：HANDOFF_IBLT_FIX.md / IBLT_FIX_HANDOFF.md / docs/A11_DEVICE_TEST_RESULTS_2026-06-21.md / docs/A11_LIVE_TEST_HANDOFF_2026-06-22.md / docs/IBLT_FIX_PLAN.md / docs/KNOWN_ISSUE_secure_storage_backup_decrypt.md — 均為別的 session 留下，非本刀產出，**不應被本 STATUS entry 涵蓋**，留給原作者自行決定 commit。
- next: Owner 雙機 A11-D2 USER-GATE 驗收（仍在 runbook 13 步排隊中）。本刀後所有功能 commit 都會被 hook 鎖住直到 STATUS commit 補完。

<!-- BEGIN CURRENT_STATE -->
## 現況總結（覆寫式 — 每刀收工以此段替換，勿 append）

**最後更新**：2026-06-22（workflow-migration @ 211248d — Claude memory 遷 Kilo + 裝 pre-commit hook 強制兩段式 commit）

**目前進度**：A0–A11 / UI-F0..F5 / UI-H0..H3-polish 皆 DONE。卡在 **A11-D2 USER-GATE**（Owner 雙機實機照 docs/ACCEPTANCE_A11_TWO_PHONE_SCRIPT.md 回填 13 步，AI 可代跑 ADB 不得代填/宣稱通過）。

**已知未解**：
- 根因③ BLE 連線 flapping → SOS 進 outbox 延遲送達（需實機抓原生 BLE log 診斷，未診斷前不瞎修）
- UI-H4 legacy cleanup（待 Owner 明確發落）

**新工作流（2026-06-22 生效，git hook 強制）**：
- 兩段式 commit 為硬性流程：功能 commit 必標記 .git/STATUS_PENDING，下個非 STATUS commit 必被擋
- 紅線：只載 .kilo/memory/no-fake-coordinate-rule.md（已刪 4 個進度類 memory 檔）
- install：換機器或 fresh clone 跑 tool/install-hooks.ps1

**下一棒**：A11-D2 Owner 雙機回填 → 全 PASS 後 A12 App↔Node 契約凍結。

**活源**：完整流水帳見本檔上方各 ## [日期] entry；commit 見 git log。
<!-- END CURRENT_STATE -->