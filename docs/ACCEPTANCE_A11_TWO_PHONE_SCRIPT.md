...
**舊包名清除（A11-fix-prep）**：本專案的 applicationId 早期曾是 bare `network.ignirelay`，
後改為現行 `network.ignirelay.field`。若手機殘留舊版 bare 包，會與正式包並存、干擾驗收。
測前先卸除任何**非** `.field` 的舊 IgniRelay 包（無殘留則 `pm uninstall` 回 `Failure` 可忽略）：

```powershell
foreach ($d in @($DEVICE_A, $DEVICE_B)) {
  # 列出目前殘留：預期只應有 network.ignirelay.field
  adb -s $d shell pm list packages | Select-String "ignirelay"
  # 卸除舊 bare 包（若不存在會 Failure，忽略即可）
...
| 9 | HAZARD | A 端危害事件出現（含分頁開啟前已收到者也顯示） | ☐P ☐F | |
