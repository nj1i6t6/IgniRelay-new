...
Stage A（App 完成）
 ...
 A2..A9 ──► A10 mapless 位置呈現 ──► A10b 雷達視圖 ──► A11 雙機驗證(USER-GATE) ──► A12 App↔Node 契約凍結
...
| 9 | A/B 相距 ≥20m 開雷達視圖（v1.2） | 對方點的方位與實際方向一致（目視粗略即可）、距離量級正確、SOS 點為 sos 色 | 截圖×2 |
...
1. A0–A10、A10b、A12 全 DONE（含證據）；A11 Owner 回填全過。
