...
| Gateway | `C:\Users\radio\Downloads\IDE\ignirelay-gateway` | LoRa 彙整 + SQLite + HTTP API + Web 管理後台（Python） |
...
3. `docs/REBUILD_PLAN.md`、`docs/PHASE0B4_WIRE_DESIGN.md`、`docs/PHASE3_MODE_B_FIELD_NODE_PLAN.md`
   — 背景與設計依據；與本文件牴觸時以本文件為準（本文件已吸收其結論）。
4. 各 repo README / STATUS — 現況描述，不是規格。

### 0.3 Owner 指定的總順序（不得擅自重排）

1. **Stage A：先把 App 做完**（含基線修復、PRESENCE/SOS/HAZARD 接線、場域加入、SOS UX、雙機驗證、契約凍結）。
2. **Stage B：用模擬器把 Field Node 與 Gateway 跑起來**（先不買任何硬體；全部在 PC 上以模擬方式讓 AGENT 開發與測試）。
3. **Stage C：Web 管理端**——跑在 PC 上、與 Gateway 同一個區域網路，使用者用瀏覽器觀看。
...
**在完全沒有行動網路、沒有網際網路的場域（登山步道、礦場、工地、營地、巡檢路線），
讓每個帶手機的人「被看見、能求救、留下最後足跡」，並讓管理者在現場的本地網頁後台
即時看到全場狀態、匯出紀錄。**
...
1. Stage A Exit（§5.13）全項通過：App 在兩台 Android 實機上完成場域加入、
   PRESENCE/SOS/HAZARD/CHECKPOINT 收發、mapless 位置呈現，所有自動化 gate 綠。
...
3. Stage C Exit（§7.8）全項通過：瀏覽器在同 LAN 另一台（或同一台）PC 上，
   ...
   匯出可下載；全程零外網請求。
4. Stage D Exit（§8）全項通過：實體 2+ 節點 bench 對傳、真手機→真 Node→LoRa→Gateway
   →Web 全鏈路 demo、戶外場試報告產出。
5. 所有「凍結契約」檔案（附錄 B）版本一致、conformance/vectors 三端（Dart/Kotlin/C/Python）對齊。
...
Stage A（App 完成）
 ...
 A2..A9 ──► A10 mapless 位置呈現 ──► A11 雙機驗證(USER-GATE) ──► A12 App↔Node 契約凍結
...
2. **QR 內容格式（凍結，v1）**：`IGNI1:<base64url(field_join_secret 32B)>:<urlencode(displayName)>`
   ——版本前綴 `IGNI1` 必驗，未知前綴拒收並提示。
3. UI：場域頁（建立場域→顯示 QR；掃碼加入；代碼輸入 fallback=A5 的 hex 對話框升級）；
   多場域清單 + 切換作用場域 + 離開場域（刪 secret + session row；**離開不可逆**需二次確認）。
4. 測試：QR 字串編解碼 roundtrip、壞前綴/壞長度拒收、多場域切換後 publisher fieldId 跟著換。
...
AGENT 工作 = 產出 `docs/ACCEPTANCE_A11_TWO_PHONE_SCRIPT.md`，內容必須含逐步操作腳本與
證據表格；Owner 實機執行回填。腳本至少涵蓋：

| # | 步驟 | 預期 | 證據欄 |
|---|---|---|---|
| 1 | 手機 A 建立場域、出示 QR；手機 B 掃碼加入 | 兩機顯示同 fieldId 前 8 碼 | 截圖×2 |
| 2 | A/B 啟動 mesh，互相發 PRESENCE | 對方位置卡出現本機 anon8 + 時間 | 截圖 |
| 3 | A 長按發 SOS(RED)（倒數中取消一次，再真發） | B 端 sosAlerts 告警卡 ≤10s | 截圖+秒數 |
| 4 | A 發「我安全了」 | B 端該 SOS 標記解除 | 截圖 |
| 5 | B 發 HAZARD（typed） | A 端危害列表出現 | 截圖 |
| 6 | 殺掉 B app 進程重啟 | 事件不重複（envelope_id dedup）、Outbox 補送 | 截圖+log |
| 7 | 手機 C（或改 B 的場域）發事件 | A **收不到**（field-scope）；trace 顯示 mismatch | trace 截圖 |
| 8 | （有環境）GATE-KOTLIN-RUN 於其中一機 | 全綠 | 輸出貼上 |
...
1. A0–A10、A12 全 DONE（含證據）；A11 Owner 回填全過。
...
## §8 Stage D — 實體硬體（前置：B10 全過 + Owner 完成採購）
...
| 單人 Owner 簽核瓶頸 | 進度阻塞 | 簽核點集中在 A12/B1/C1/B9 例外/各 Stage Exit |
...
## 附錄 D — Gateway API v1 草案：見 §7 C1 表（C1 落定 normative）。
